...
static int send_packet_to_client(struct dhcpMessage *payload, int force_broadcast)
{
	...
	if (force_broadcast || (payload->flags & htons(BROADCAST_FLAG)) || payload->ciaddr == 0)
	{
...
#elif (WIFI_SUPPORT_VERSION == BOARD_WIFI_SV6030) || (WIFI_SUPPORT_VERSION == BOARD_WIFI_SV6152)||(WIFI_SUPPORT_VERSION ==BOARD_WIFI_SV6158)
	add_simple_option(packet->options, DHCP_DNS_SERVER, dns_getserver(0).addr);
#else
	add_simple_option(packet->options, DHCP_DNS_SERVER, dns_getserver(0)->addr);
