...
struct mutex dhcp_mutex;
extern struct netif *p_netif;
int udhcpd_sock;
void* udhcpd_event = NULL;

#define DHCP_RCV_SIZE 1024
void udp_dhcpd_read(struct event_info *e, void *d)
{
	char *tcp_rcv_buff;
	...
	tcp_rcv_buff = (char *)mem_malloc(DHCP_RCV_SIZE + 4);

	size = recvfrom(udhcpd_sock, tcp_rcv_buff, DHCP_RCV_SIZE, MSG_DONTWAIT, &remote_addr, (socklen_t *)&retval);

	if (size > 0)
	{
		handle_dhcp_rcv((char *)tcp_rcv_buff, size);
	}

	mem_free(tcp_rcv_buff);
}
...
int init_udhcpd()
{
	...
	server_config.start = inet_addr("192.168.66.2");
	server_config.end = inet_addr("192.168.66.64");
	server_config.lease = LEASE_TIME;
	server_config.max_leases = 10; //10
	server_config.min_lease = 10;
	...
	mutex_init(&dhcp_mutex);

	// mutex_lock(&dhcp_mutex);
	socket_s = socket(AF_INET, SOCK_DGRAM, 0);
	if (socket_s == -1)
	{
		// mutex_unlock(&dhcp_mutex);
		p_err("1 ");
		return -1;
	}
	/*
	...
	udhcpd_sock = socket_s;
	server_config.inited = TRUE;
	// mutex_unlock(&dhcp_mutex);

	udhcpd_event = eloop_add_fd(udhcpd_sock, 0, 0, udp_dhcpd_read, 0);

	return 0;
}

void release_udhcpd1(void* e,u32* work)
{
	if(udhcpd_event)
		eloop_remove_event(udhcpd_event);
	udhcpd_event = NULL;
	disable_dhcp_server();
	socket_stop_select(udhcpd_sock);
	closesocket(udhcpd_sock);
	udhcpd_sock=-1;
	*work=0;
}
void release_udhcpd(void)
{
	u32 working=1,i;
	eloop_add_alarm(0,0,release_udhcpd1,&working);
	i=0;
	while(working)
	{
		osal_task_sleep(10);
...
int handle_dhcp_rcv(uint8_t *buff, int size)
{
	...
	printf("DHCP client:%02X:%02X:%02X:%02X:%02X:%02X\n",dhcp_packet->chaddr[0],dhcp_packet->chaddr[1],dhcp_packet->chaddr[2],dhcp_packet->chaddr[3],dhcp_packet->chaddr[4],dhcp_packet->chaddr[5]);
	if(lease)
	{
		printf("DHCP lease:%d,%d\n",time(0),lease->expires);
	}
	switch (state[0])
	{
	case DHCPDISCOVER:
		DEBUG(LOG_INFO, "received DISCOVER");
		printf("DHCPDISCOVER,0X%X,0X%X,%d\n",dhcp_packet->ciaddr,dhcp_packet->yiaddr,dhcp_packet->secs);
	...
	case DHCPREQUEST:
		...
		printf("DHCPREQUEST,0X%X,0X%X,%d\n",dhcp_packet->ciaddr,dhcp_packet->yiaddr,dhcp_packet->secs);
		...
		printf("request:%08X,server_id:%08X\n", requested_align, server_id_align);
		...
		if (0 == server_id) /* if no nak, will wait to timeout, Iphone will retry 3 times */
		{
			sendNAK(dhcp_packet);
		}
		else
	...
	case DHCPDECLINE:
		...
		printf("DHCPDECLINE,0X%x,0X%x,%d\n",dhcp_packet->ciaddr,dhcp_packet->yiaddr,dhcp_packet->secs);
		if (lease)
		{
			memset(lease->chaddr, 0, 16);
	...
	case DHCPRELEASE:
		...
		printf("DHCPRELEASE,0X%x,0X%x,%d\n",dhcp_packet->ciaddr,dhcp_packet->yiaddr,dhcp_packet->secs);
		if (lease)
