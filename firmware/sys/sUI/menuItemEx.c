...
winHandle itemCreateMenuItemEx(int16 x0,int16 y0,uint16 width,uint16 height)
{
	...
	if(hmenuItemEx!=INVALID_HANDLE)
	{
		...
		pmenuItemEx->hStr=stringIconCreate(x0+height,y0,(width-(height<<1))>>1,height,INVALID_HANDLE,WIN_NOT_ZOOM,INVALID_WIDGET_ID);
		pmenuItemEx->hSelStr=stringIconCreate(x0+height+((width-(height<<1))>>1),y0,(width-(height<<1))>>1,height,INVALID_HANDLE,WIN_NOT_ZOOM,INVALID_WIDGET_ID);
