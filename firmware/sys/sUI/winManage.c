...
void winDestroy(winHandle* hWin)
{
	...
	if(pWin->style&WIN_WIDGET)
	{
		...
		if(pWin==NULL)
		{
			debug_wrn("can not find this widget\n");
			uiUnlock();
			return;
		}
...
uint8 winPaint(void)
{
	...
	if(state.mempError)
	{
		...
		debug_msg("memPool error information:\n");
		mempInfor(&rectMemp);
	}
...
void winInit(void)
{
	#define WIN_HEAP_SIZE  (1024*40)
	static uint8 winHeap[WIN_HEAP_SIZE];
	lcdGetOsdRes(&LCD_WIDTH,&LCD_HEIGHT);
