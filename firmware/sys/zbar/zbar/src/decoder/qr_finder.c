...
inline zbar_symbol_type_t _zbar_find_qr (zbar_decoder_t *dcode)
{
	...
	unsigned qz = get_width(dcode, 0);
	unsigned w = get_width(dcode, 1);
	unsigned hal_w=(w + 1)>>1;
	qrf->line.eoffs = qz + hal_w;
	qrf->line.len = qz + w + get_width(dcode, 2);
	qrf->line.pos[0] = qrf->line.len + get_width(dcode, 3);
	qrf->line.pos[1] = qrf->line.pos[0];
	w = get_width(dcode, 5);
	qrf->line.boffs = qrf->line.pos[0] + get_width(dcode, 4) + hal_w;
#endif
