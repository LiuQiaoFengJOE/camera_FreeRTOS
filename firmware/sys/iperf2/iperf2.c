...
static int iperf_start(void *nif, int handle);
...
#define malloc(x)   osal_malloc(x)
#define free(x)     osal_free(x)


//#define    INADDR_NONE        ((uint32_t) 0xffffffff)

#define IPERF_THREAD_PRIO           22
#define IPERF_THREAD_STACK_SIZE     2048

#define iperf_thread_exit(thread)   vTaskDelete(*(thread))
#define iperf_errno                 0
#define iperf_msleep(msec)          vTaskDelay(msec)
...
// in 1ms
#define IPERF_TIME_PER_SEC         configTICK_RATE_HZ
#define IPERF_TIME()               xTaskGetTickCount()
#define IPERF_SEC_2_INTERVAL(sec)  ((sec) * IPERF_TIME_PER_SEC)

#define IPERF_SELECT_TIMEOUT        100

static iperf_arg *g_iperf_arg_handle[IPERF_ARG_HANDLE_MAX] = {NULL};
static uint8_t g_iperf_task_not_running[IPERF_ARG_HANDLE_MAX] = {0};

// 为每个任务提供一个静态缓冲区
static uint8_t g_iperf_static_buf[IPERF_ARG_HANDLE_MAX][IPERF_BUF_SIZE];
static uint8_t g_iperf_buf_used[IPERF_ARG_HANDLE_MAX] = {0};

static void iperf_speed_log(iperf_arg *arg, uint64_t bytes, uint32_t time,
                            int8_t is_end)
{
    uint64_t speed;
    uint32_t integer_part, decimal_part,tmp_tras;
    char str[16];
	...
	tmp_tras = speed & 0xffffffff;
    integer_part = tmp_tras/100;
    decimal_part = tmp_tras % 100;

    if (integer_part >= 100) {
        sprintf(str, "%u", integer_part);
    } else if (integer_part >= 10) {
        sprintf(str, "%u.%u", integer_part, decimal_part / 10);
    } else {
        sprintf(str, "%u.%02u", integer_part, decimal_part);
    }
	//IPERF_LOG(1,"speed:%u,integer_part:%u,decimal_part:%u\n",tmp_tras,integer_part,decimal_part);

    IPERF_LOG(1, "[%d] %u.%u-%u.%u sec %s%s %s\n", arg->handle, is_end ?  0 : (((iperf_arg *)arg)->sec_count - ((iperf_arg *)arg)->interval), 0, \
              is_end ?  (time / IPERF_TIME_PER_SEC) : (((iperf_arg *)arg)->sec_count), is_end ?  ((time - (time / IPERF_TIME_PER_SEC) * IPERF_TIME_PER_SEC ) / 100) : 0, \
              is_end ?  "TEST END: " : "", str, (arg->flags & IPERF_FLAG_FORMAT) ? "KB/s" : "Mb/s");
}
...
static uint8_t *iperf_buf_new(uint32_t size)
{
    ...
    for (i = 0; i < IPERF_ARG_HANDLE_MAX; i++) {
        if (g_iperf_buf_used[i] == 0) {
            g_iperf_buf_used[i] = 1;
            
            // 初始化缓冲区内容
            for (j = 0; j < size && j < IPERF_BUF_SIZE; j++) {
                g_iperf_static_buf[i][j] = '0' + j % 10;
            }
            
            return g_iperf_static_buf[i];
        }
    }
    
    // 如果没有可用缓冲区，返回NULL
    IPERF_ERR("No available static buffer!\n");
    return NULL;
}

// 新增函数：释放静态缓冲区
static void iperf_buf_free(uint8_t *buf)
{
    uint32_t i;
    ...
    for (i = 0; i < IPERF_ARG_HANDLE_MAX; i++) {
        if (buf == g_iperf_static_buf[i]) {
            g_iperf_buf_used[i] = 0;
            break;
        }
    }
}
...
#define iperf_calc_speed()                                                    \
    ...
    cur_tm = IPERF_TIME();                                                    \
    if (cur_tm >= end_tm) {                                                    \
        ((iperf_arg *)arg)->sec_count += ((iperf_arg *)arg)->interval;       \
        iperf_speed_log((iperf_arg *)arg, data_cnt, cur_tm - beg_tm, 0);     \
        data_cnt = 0;                                                        \
        beg_tm = IPERF_TIME();                                               \
        end_tm = beg_tm + IPERF_SEC_2_INTERVAL(((iperf_arg *)arg)->interval) - (cur_tm - end_tm);\
        run_beg_tm += (beg_tm - cur_tm);                                     \
        run_end_tm += (beg_tm - cur_tm);                                     \
    }                                                                        \
    if (idata->mode_time) {                                                    \
...
socket_error:
    if (data_buf)
        iperf_buf_free(data_buf);
    if (local_sock >= 0)
        lwip_close(local_sock);

    iperf_thread_hand_t thread = idata->iperf_thread;
    handle = idata->handle;
...
static const iperf_thread_function_t iperf_thread_entry[IPERF_MODE_NUM] = {
    iperf_udp_send_task,
    iperf_udp_recv_task,
    iperf_tcp_send_task,
    iperf_tcp_recv_task,
...
int iperf_handle_start(void *nif, int handle)
{
    ...
    iperf_arg * iperf_arg = g_iperf_arg_handle[handle];
    if (iperf_arg->iperf_thread) {
        IPERF_ERR("iperf task (%s) is running\n", iperf_mode_str[iperf_arg->mode]);
        return -1;
    }

    /* clean stop flag */
    iperf_arg->flags &= ~IPERF_FLAG_STOP;
    g_iperf_task_not_running[handle] = 0;

    if (xTaskCreate(iperf_thread_entry[iperf_arg->mode],
        iperf_mode_str[iperf_arg->mode],
        IPERF_THREAD_STACK_SIZE,
        iperf_arg,
        IPERF_THREAD_PRIO,
        &iperf_arg->iperf_thread) != pdPASS)
    {
        IPERF_ERR("iperf task (%s) create failed\n", iperf_mode_str[iperf_arg->mode]);
        return -1;
    }
...
{
    int i = 0;
    iperf_thread_hand_t iperf_thread;

    if (handle < 0)
        return -1;

    if ((handle < IPERF_ARG_HANDLE_MAX) && (g_iperf_arg_handle[handle] != NULL)) {
        if ((g_iperf_arg_handle[handle]->iperf_thread)) {
...
int iperf_stop(char* arg)
{
    int handle = 0;
    if (strcmp("a", arg) == 0){
		IPERF_DBG("close all handle\n");
		return iperf_handle_stop(IPERF_ARG_HANDLE_MAX);
	}else{
		IPERF_DBG("close index:%s \n", arg);
	}
...
int iperf_parse_argv(int argc, char *argv[])
{
	...
	IPERF_DBG("\iperf_parse_argv argc:%d, cmd:\r\n",argc);
	for(i=0;i<argc;i++)
		printf("%s ",argv[i]);
    ...
    for (i = 1; i < argc; i++) {
        if (argv[i][0] != '-') {
            IPERF_ERR("Error: Non-option argument %s\n", argv[i]);
            return -1;
        }
        switch (argv[i][1]) {
            case 'L':
            ...
            case 'c':
                f_value = argv[++i];
                snprintf(iperf_arg_t.remote_ip, IPERF_ADDR_STRLEN_MAX, "%s", f_value);
                iperf_arg_t.flags |= IPERF_FLAG_CLINET;
                break;
            case 'f':
                f_value = argv[++i];
                if (strcmp("m", f_value) == 0 )
                    iperf_arg_t.flags &= ~IPERF_FLAG_FORMAT;
                else if (strcmp("K", f_value) == 0)
                    iperf_arg_t.flags |= IPERF_FLAG_FORMAT;
                else {
            ...
            case 't':
                f_value = argv[++i];
                iperf_arg_t.run_time = (uint32_t)atoi(f_value);
                break;
            case 'i':
                f_value = argv[++i];
                iperf_arg_t.interval = (uint32_t)atoi(f_value);
                break;
#if IPERF_OPT_BANDWIDTH
            case 'b': {
                uint32_t value;
                f_value = argv[++i];
                char suffix = '\0';
                sscanf(f_value, "%u%c", &value, &suffix);
            ...
            case 'S':
                f_value = argv[++i];
                iperf_arg_t.tos = (uint16_t)strtol(f_value, NULL, 0);
                break;
#endif
            case 'B':
                f_value = argv[++i];
                snprintf(iperf_arg_t.host_ip, IPERF_ADDR_STRLEN_MAX, "%s", f_value);
                break;
            default:
                IPERF_ERR("Error: Unknown option %s\n", argv[i]);
                return -1;
        }
    }
...
void iperf2(int argc, char **argv)
{
