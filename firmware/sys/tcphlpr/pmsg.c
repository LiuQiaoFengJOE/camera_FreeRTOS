...
char *pmsg_add_string(struct pmsg *msg, char *s)
{
  ...
  if (msg->msg_len + len > msg->max_len)
    return NULL;
...
static int inline delim(char *d, int len, char term1, char term2)
{
  ...
  for (s = 0; s < len; ++s)
    if (d[s] == term1 || d[s] == term2 || d[s] == '\r' || d[s] == '\n')
      break;
  return s;
}
...
int pmsg_parse(struct pmsg *msg)
{
  ...
  if (d[i] == '/')
  {
    ...
    if (len - i < 4 || !isdigit(d[i]) || !isdigit(d[i + 1]) || !isdigit(d[i + 2]) || d[i + 3] != ' ')
      return -1;
...
{
  if (msg->header_count == MAX_FIELDS)
    return -1;
  /* Put the name and value back-to-back at the end of the message */
  msg->fields[msg->header_count].name = pmsg_add_string(msg, name);
  if (!msg->fields[msg->header_count].name)
    return -1;
  msg->fields[msg->header_count].value = pmsg_add_string(msg, value);
  if (!msg->fields[msg->header_count].value)
    return -1;
  ++msg->header_count;
  return 0;
}
...
int pmsg_add_header_printf(struct pmsg *msg, char *name, char *fmt, ...)
{
  ...
  va_start(ap, fmt);
  len = vsnprintf((char *)msg->msg + msg->msg_len, msg->max_len - msg->msg_len, fmt, ap);
  va_end(ap);
...
int pmsg_replace_header(struct pmsg *msg, char *name, char *value)
{
  ...
  for (i = 0; i < msg->header_count; ++i)
    if (!strcasecmp(msg->fields[i].name, name))
    {
      ...
      {
        char *p = pmsg_add_string(msg, value);
        if (!p)
          return -1;
        msg->fields[i].value = p;
      }
      else
        strcpy(msg->fields[i].value, value);
      return 0;
    }
  /* It doesn't exist, so we'll insert it as new */
  return pmsg_add_header(msg, name, value);
}
...
int pmsg_copy_headers(struct pmsg *dest, struct pmsg *src, char *name)
{
  ...
  for (i = 0; i < src->header_count; ++i)
    ...
    {
      pmsg_add_header(dest, src->fields[i].name, src->fields[i].value);
      ++count;
    }
  return count;
}
...
struct pmsg *pmsg_new(int size)
{
  ...
  if (!(v = osal_malloc(sizeof(struct pmsg) + size)))
  {
    p_err("unable to allocate memory for message");
    return NULL;
  }
  msg = (struct pmsg *)v;
