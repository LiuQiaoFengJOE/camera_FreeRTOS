...
extern int8_t console_getc();
extern int8_t console_data_ready();

static char rt_hw_console_getchar(void)
{
...
#ifdef RT_USING_HEAP
    result = xTaskCreate(finsh_thread_entry, FINSH_THREAD_NAME,
                                  configMINIMAL_STACK_SIZE * 8, NULL,
                                  tskIDLE_PRIORITY + 1, &tid);

    if (result != pdPASS) {
        rt_kprintf("Thread creation failure\n");
    }
