...
typedef struct _Sensor_Adpt_
{
	void (*iic_write)(void *); // 由初始化程序配置
	uint32_t (*iic_read)(void *);
	void (*sen_debg)(u32 data_cal);
...
#define sen_iic_write(p) ((_Sensor_Adpt_ *)sensor)->iic_write(p)
#define sen_iic_read(p) ((_Sensor_Adpt_ *)sensor)->iic_read(p)
#define sen_printf(p) ((_Sensor_Adpt_ *)sensor)->sen_debg(p)
typedef struct
{
...
typedef struct
{
	...
	u32 header_item_size; // no use
	u32 align[13];
} _Sensor_Header_;
typedef struct
{
	void *contrast_tab_adr;
	u32 contrast_tab_size;
	void *gamma_tab_adr;
	u32 gamma_tab_size;
} _Sensor_Common_;
