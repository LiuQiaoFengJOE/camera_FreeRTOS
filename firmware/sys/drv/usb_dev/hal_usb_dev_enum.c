...
{
	memset((u8 *)&usb_device, 0, sizeof(USB_DEV));
}
bool usbdev_rqu(void)
{
	return usb_device.connect;	
}
void usbdev_reset(void)
{
	usb_device.connect = false;
}
/*******************************************************************************
* Function Name  : timer_Timer1_Stop
* Description    : timer1 stop
...
{
	usb_device.ep1_tx_stall = 0;
    usb_device.ep1_rx_stall = 0;
    usb_device.ep1_pipe_stall = 0;
...
{
    if (usb_device.set_addr) {
        usb_device.set_addr = 0;
...
bool usb_ep0_clrrx_pkt0(void)
{
    ...
    usb_device.ep0_state = TRANSER_OVER_STATE;
    return true;
}
...
void usb_stall_ep(u8 epnum)
{
    ...
    case 0x00:
        ...
        break;
    case 0x01:
        usb_device.ep1_rx_stall = 1;
        USB20_OutCSR1 = BIT(5);           //Stall
        break;
    case 0x81:
        usb_device.ep1_tx_stall = 1;
...
{
    usb_device.ep1_pipe_stall = 1;
	usb_stall_ep(epout);
	usb_stall_ep(epin);
	while(usb_device.ep1_pipe_stall) {
        if(usb_device.ep1_rx_stall == 0) {
            usb_stall_ep(0x01);
		}
        if(usb_device.ep1_tx_stall == 0) {
    		usb_stall_ep(0x81);
		}
    }
}
...
{
	memcpy((u8 *)&usb_rqu, usb_device.ep0_buf, 8);
	usb_device.connect = true;
#ifdef _DEBG_USB_ENUM__
    //printf("request:");
	printfbuf(usb_device.ep0_buf,8);
#endif
}
/*******************************************************************************
* Function Name  : timer_Timer1_Stop
...
bool usb_ep0_transfer(void)
{
    u8 cmd, len;
	//USB20_Index = 0x00;
    len = usb_device.ep0_len;
    if (usb_device.ep0_len) {
        if (usb_device.ep0_len > usb_device.ep0_pkt) {
            len = usb_device.ep0_pkt;
        }
        usb_device.ep0_len -= len;
        //printfbuf(usb_device.ep0_ptr,len);
        memcpy(usb_device.ep0_buf, usb_device.ep0_ptr, len);
		usb_device.ep0_ptr += len;
		USB20E0ADR = (u32)usb_device.ep0_buf;
		USB20CON1 = BIT(13 + 0) + len;
    }
    cmd = BIT(1);
    if (len != usb_device.ep0_pkt) {
        cmd |= BIT(3);
        usb_device.ep0_state = TRANSER_OVER_STATE;
		//printf("TRANSER_OVER_STATE\n");
    }
    USB20_CSR0 = cmd;  //DataEnd, TxRdy
    //while(read_usb_reg(CSR0) & BIT(1));
	//printfbuf(ep0buf,len);
    return true;
}
...
bool usb_ep0_tx(u8* txbuf, u8 len)
{
	...
	//printfbuf(txbuf,len);
	memcpy(usb_device.ep0_buf, txbuf, len);
	//USB20_FIFO0 = usb_device.ep0_buf[0];
	//USB20CON1 &= ~(0x1fff);
	//USB20CON1 |= len;
	//USB20CON1 |= BIT(13 + 0);
	USB20E0ADR = (u32)usb_device.ep0_buf;
...
{	
	...
	if(NULL != pSetReqCallback) 
		(*pSetReqCallback)(usb_device.ep0_buf);
...
bool usb_ep0_get_status(void)
{
    ...
    if (USB_DEFAULT_STATE == usb_device.cur_state) {
        return false;
    }
    usb_device.ep0_buf[0] = 0;
    ...
    case REQUEST_TO_INTERFACE:
        if (USB_ADDRESS_STATE == usb_device.cur_state) {
            return false;
        }
        break;                                 //æ¥å£ç´æ¥å?
    case REQUEST_TO_ENDPOINT:
        //æ¥è¯¢ç«¯ç¹æ¯å¦è¢«Stall
        if (USB_ADDRESS_STATE == usb_device.cur_state && 0 != USB_REQUEST_ENDPOINT) {
            return false;
        }

        if ((USB_REQUEST_ENDPOINT & 0x7f) <= MAX_EP_D) {
            if (USB_REQUEST_ENDPOINT & 0x80) {
                if (usb_device.ep1_tx_stall) {
                    usb_device.ep0_buf[0] = 1;
                }
            } else {
                if (usb_device.ep1_rx_stall) {
                    usb_device.ep0_buf[0] = 1;
                }
            }
        }
    ...
    usb_device.ep0_buf[1] = 0;                         //é«Byteå?
    return usb_ep0_tx(usb_device.ep0_buf,2);
}
/*******************************************************************************
* Function Name  : timer_Timer1_Stop
* Description    : timer1 stop
...
bool usb_ep0_clr_feature(void)
{
    if (USB_DEFAULT_STATE != usb_device.cur_state &&
      ...
      !(USB_ADDRESS_STATE == usb_device.cur_state && USB_REQUEST_ENDPOINT)) {
...
{
    switch (usb_device.cur_state) {
    case USB_CONFIG_STATE:
        return false;
    case USB_DEFAULT_STATE:
        if (usb_rqu.value) {
            usb_device.cur_state = USB_ADDRESS_STATE;
        }
        break;
    case USB_ADDRESS_STATE:
        if (usb_rqu.value == 0) {
            usb_device.cur_state = USB_DEFAULT_STATE;
        }
        break;
    }
    usb_device.set_addr = 1;
    return usb_ep0_clrrx_pkt0();
}
/*******************************************************************************
* Function Name  : timer_Timer1_Stop
...
{
    if (USB_DEFAULT_STATE != usb_device.cur_state) {
        usb_device.ep0_buf[0] = usb_device.cfg_value;
        return usb_ep0_tx(usb_device.ep0_buf,1);
    }
    return false;
}
/*******************************************************************************
...
bool usb_ep0_set_configuration(void)
{
    ...
    if (USB_DEFAULT_STATE != usb_device.cur_state) {
        ...
        if (usb_rqu.value <= 1) {
            usb_device.cfg_value = usb_rqu.value;
            if (usb_device.cfg_value == 0) {
                usb_device.cur_state = USB_ADDRESS_STATE;
            } else {
                usb_device.cur_state = USB_CONFIG_STATE;
				//printf("USB_CONFIG_STATE\n");
            }
...
{
    if (USB_CONFIG_STATE == usb_device.cur_state) {
        usb_device.ep0_buf[0] = 0;//usb_config_all_descriptor[12];
 ...
 bool usb_ep0_set_interface(void)
{
    ...
    if (USB_CONFIG_STATE == usb_device.cur_state) {
        ...
        if (usb_device.uac_tx_interface == usb_rqu.index) {
            //æ¯æ¬¡å¼å¯åå³é­MICä¹åï¼é½ä¼set interface
            if (usb_rqu.value) {
				printf("UAC ON\n");
				(*fp_uac_on_hal)();
				uac_onflg = 1;
                usb_device.uac_tx_enable_int = 1;
            } else {
				printf("UAC OFF\n");
				(*fp_uac_off_hal)();
				uac_onflg = 0;
                usb_device.uac_tx_enable_int = 0;
            }
        }

        if (usb_device.uvc_tx_interface == usb_rqu.index) {
            //æ¯æ¬¡å¼å¯åå³é­MICä¹åï¼é½ä¼set interface
            if (usb_rqu.value) {
				printf("UVC ON\n");			
				//(*uvc_on_hal)();
				...
				uvc_onflg = 1;
                usb_device.uvc_tx_enable_int = 1;
            } else {
				printf("UVC OFF\n");
				uvc_onflg = 0;
				//(*uvc_off_hal)();
				__FP__(fp_uvc_off_hal);
                usb_device.uvc_tx_enable_int = 0;
...
bool usb_ep0_descriptor(void *buf, u16 len)
{
    ...
    usb_device.ep0_ptr = (u8 *)buf;
    usb_device.ep0_len = len;
	//printf("len:");
	//printfbuf(&len,2);
    usb_device.ep0_state = TRANSFERING_STATE;
    USB20_CSR0 = BIT(6);           //æ¸é¤RxRdy
	//printf("usb_ep0_transfer\n");
...
bool usb_ep0_standard(void)
{
    ...
    case 0x05:     //Set Address
		//printf("-usb set adr\n");
        //usb_device.ep0_pkt = 64;//tbl_usb_device_descriptor[7];         //è®¾å°ååï¼è®¾ç½®åå¤§å°?
...
void usb_ep0_hal(void)
{
    ...
    if (csr0 & BIT(2)) {                        //SentStall
        USB20_CSR0 = 0;
        usb_device.ep0_state = IDLE_STATE;
    }
    ...
    if (usb_device.ep0_state == TRANSER_OVER_STATE) {
        usb_device.ep0_state = IDLE_STATE;
    }
	//printf("- csr0:%x\n",csr0);
	
    switch (usb_device.ep0_state) {
    case IDLE_STATE:
...
{
	usb_device.ep0_buf = _ep0_fifo_;
    usb_device.ep0_pkt = 64;                    //初始包大小
    usb_device.type = devtyp;
	usb_device.uvc_tx_interface = __UVC_STRM_INTFS__;
	usb_device.uac_tx_interface = __UAC_STRM_INTFS__;
...
void usb_dev_enum_reg(u8 tpe)
{
	...
	//uvc
		...
		usbdsc.pcfg = uvc_cfg;
		usbdsc.cfglen = sizeof(uvc_cfg); 

		printf("USB device = UVC\n");
	}else if(1 == tpe){
		usbdsc.pdev = TYPE_COMB_DEVICE;
		usbdsc.devlen = sizeof(TYPE_COMB_DEVICE);
		usbdsc.pcfg = TYPE_COMB_CONFIG;
		usbdsc.cfglen = sizeof(TYPE_COMB_CONFIG); 

		printf("USB device = UVC+UAC+MSC\n");		
	}
