...
#define FUCTION_NUM 6 //对应reset.s中的0x140地址的对应函数
#define LSB(a,b,c,d) (((u32)d << 24) | ((u32)c << 16) | ((u32)b << 8) | ((u32)a << 0))

extern void Sensor_DebugRegister(u8 *pbdata ,u32 trans_len, u32 param_3B);
extern void Sensor_common_Get(u8 *pbdata ,u32 trans_len, u32 param_3B);
extern void Sensor_common_Set(u8 *pbdata ,u32 trans_len, u32 param_3B);
extern void isp_debug_write(u8 *pbdata ,u32 trans_len, u32 param_3B);
extern void isp_debug_read(u8 *pbdata ,u32 trans_len, u32 param_3B);
extern void usb_cut_raw(u8 *pbdata ,u32 trans_len, u32 param_3B);
void funcIspNull(u8 *pbdata ,u32 trans_len, u32 param_3B)
{
}
static u32 funcIspBuff[512/4];
void cb_ram_func_init(void)
{
	memcpy(funcIspBuff,"SFAT",4);
	funcIspBuff[(0x20/4)]=cb_FIFO2mem;
	funcIspBuff[(0x20/4)+1]=cb_mem2FIFO;

	funcIspBuff[(0x40/4)]=Sensor_DebugRegister;
	funcIspBuff[(0x40/4)+1]=Sensor_common_Get;
	funcIspBuff[(0x40/4)+2]=Sensor_common_Set;
	funcIspBuff[(0x40/4)+3]=isp_debug_write;
	funcIspBuff[(0x40/4)+4]=isp_debug_read;
	funcIspBuff[(0x40/4)+5]=usb_cut_raw;

	funcIspBuff[(0x60/4)]=funcIspNull;
	funcIspBuff[(0x60/4)+1]=funcIspNull;
	funcIspBuff[(0x60/4)+2]=funcIspNull;
	funcIspBuff[(0x60/4)+3]=funcIspNull;

	funcIspBuff[(0x70/4)]=funcIspNull ;
}
void cb_mem_read (void) {

    if(0xffffffff == MscCmd.DataAddr) 
		MscCmd.DataAddr = (uint32)scsi.ptxbuf;
	// 先L2_func再讀內存
	if (MscCmd.Func2 + 1) {
		l2_func L2_func = (l2_func)MscCmd.Func2;
        L2_func (MscCmd.DataAddr, MscCmd.Residue, MscCmd.Param);
	}
	if(MscCmd.DataAddr==0x100)
    	cb_mem2FIFO ((uint8_t *)funcIspBuff, MscCmd.Residue);
	else
...
void mscCmd_ufmod(void)
{
   ...
   if(MscCmd.Func1 == 0x110){
		cb_mem_write();
	}else if(MscCmd.Func1 == 0x118){
		cb_mem_read();
	}
...
bool scsi_cmd_analysis (void)
{
	...
	if(MscCmd.CbwTrxLength == 0)
	{
		...
		if (MscCmd.OpCode == 0x00) {
			...
			u8 *prxbuf = scsi.pcbw;
			//printf("MscCmd.SubOpCode=0x%x\n",MscCmd.SubOpCode);
			//printf("prxbuf[15]=0x%x,prxbuf[16]=0x%x,prxbuf[17]=0x%x,prxbuf[18]=0x%x\n",prxbuf[15],prxbuf[16],prxbuf[17],prxbuf[18]);
	...
	//data in
		...
		if(MscCmd.OpCode == 0x28) {
			...
			mscCmd_ufmod();
		} else if (MscCmd.OpCode == 0xAC) {
			printf("\r\r\r\nrun in 0xAC\n");
			ACHandlerFunction();
		} else if (MscCmd.OpCode == 0xBC) {
			printf("\r\r\r\nrun in 0xBC\n");
			BCHandlerFunction();
		} else if (MscCmd.OpCode == 0xDC) {
			printf("\r\r\r\nrun in 0xDC\n");
			DCHandlerFunction();
		} else if (MscCmd.OpCode == 0xFC) {
			printf("\r\r\r\nrun in 0xFC\n");
			FCHandlerFunction();
		}
		else{
	...
	//data out
		...
		if (MscCmd.OpCode == 0x2A) {
			...
			mscCmd_ufmod();
		} else if (MscCmd.OpCode == 0xAD) {
			printf("\r\r\r\nrun in 0xAD\n");
			ADHandlerFunction();
		} else if (MscCmd.OpCode == 0xEC) {
			printf("\r\r\r\nrun in 0xEC\n");
			ECHandlerFunction();
		} else {
			MscCmd.Residue = MscCmd.CbwTrxLength;
...
bool get_cbw(void)
{
	...
	//printf(".OpCode:%x,\n",MscCmd.OpCode);
	//printf("C:0x%x,f1:0x%x,f2:0x%x,D:0x%x,P:0x%x\n",MscCmd.OpCode,MscCmd.Func1,MscCmd.Func2,MscCmd.DataAddr,MscCmd.Param);
