...
extern bool usb_iso_tx(u8 epx, u32 adr, u16 len);
...
bool uvc_unit_ctl_ex_hal(u8 val, u8 rqu, u8 len)
{
	...
	if(val < __PU_TYPES__){
		...
		printf("__Err__:rqu:%x,%x\n",rqu,val);	
		return false;		
	}	
	else{
		//printf("__Err__: val:%x\n",val);	
		return false;
	}
}
...
void uvc_video_probe_control(u8 request,u8 len)
{
	if (request == UVC_SET_CUR){
		...
		usb_device.ep0_state = RECEIVING_STATE;
		...
		//printf("ack26:");debgbuf(uvc_video_probe_commit_value,sizeof(uvc_video_probe_commit_value));
...
BOOL uvc_still_probe_control_callback(u8* rxbuf)
{
	...
	//printf("set 11:");debgbuf(rxbuf,11);
	return true;
}
...
void uvc_still_probe_control(u8 request,u8 len)
{
	if (request == UVC_SET_CUR){
		...
		//printf("ack11:");debgbuf(UVC_Still_Probe_Commit,sizeof(UVC_Still_Probe_Commit));
...
#if 1
#if UVC_MOD	== UVC_MJP
	winid = uvc_video_probe_commit_value[3];
	winid -=1 ;	
	if(winid <= 4){
		uvcEnable(1);
	}
...
u32 getUvcW(void)
{
	return probe[winid].win_w;
}
u32 getUvcH(void)
{
	return probe[winid].win_h;
}
void uvc_off(void)
{
	uvcEnable(0);
