...
#define ALIGNED(x) __attribute__((align(x)))
#define __FP__(f) \
	if (f)        \
	{             \
		(*f)();   \
...
extern u32 get_time_for_tfprintf(void);
...
u16 using_ygama[512];
ALIGNED(32)
u8 using_contrast[256];
ALIGNED(32)
u8 using_rgbgama[512];
...
bool data_check(void *p_data, u32 min, u32 max)
{
	...
	{
		printf("data:%x out of range!!!\n", p_data);
		return false;
	}
}

static bool pointer_check(void *p_pointer)
{
...
void isp_isr_hal(void)
{
	...
	if (exp_gain_flag == 1)
	{
		if (pointer_check(&(csiSensor->p_fun_adapt.fp_exp_gain_wr)))
		{
			//		    printf("auto br:%d  e:%d	g:%d vyloga:%d\n",vbr,sen_exp,sen_gain,vyloga);
			(*csiSensor->p_fun_adapt.fp_exp_gain_wr)(sen_exp, sen_gain);
		}
		// printf("auto br:%d  e:%d  g:%d vyloga:%d\n",vbr,sen_exp,sen_gain,vyloga);
		exp_gain_flag = 0;
	}
...
{
	u32 i, Kr, Kg, Kb;
	u16 weight[4][8] = {{256, 0, 0, 0, 0, 0, 0, 0}, {192, 64, 0, 0, 0, 0, 0, 0}, {80, 80, 64, 32, 0, 0, 0, 0}, {24, 40, 64, 64, 40, 24, 0, 0}}; //
...
void isp_AWB_manual_adj(void)
{
	...
	isp.p_awb->bgain=isp.p_awb->manu_bgain;
	isp.p_awb->ggain=isp.p_awb->manu_ggain;
	isp.p_awb->rgain=isp.p_awb->manu_rgain;
...
void isp_LSC_adj_cfg(u16 *p_lsc_tab, u8 first_time)
{
	...
	if (first_time)
	{
		...
		//res_read(p_sen_ident->lsc_tab_adr, p_lsc_tab, p_sen_ident->lsc_tab_size);
		memcpy( p_lsc_tab,p_sen_ident->lsc_tab_adr, p_sen_ident->lsc_tab_size);
	}
...
{
	extern const u8 common_contra_tab[_CONTRA_CLSASSES_][256];
	_Sensor_Ident_ *p_sen_ident = sensor_init_get();
	if (contra_num < _CONTRA_CLSASSES_)
	{
		...
		//res_read(p_sen_ident->sensor_struct_addr + contra_num * 256, using_contrast, 256);
		memcpy(using_contrast,&common_contra_tab[contra_num], 256);
	}
	if (!(data_check(using_contrast, 0, 32) && data_check(&using_contrast[255], 224, 255)))
	{
		printf("using_contrast resource_load err");
		// while(1);
	}
	return;
}
void isp_rgbgma_table_load(u32 rgbgma_num0, u32 rgbgma_num1)
{
	...
	//res_read(p_sen_ident->rgb_gamma_tab_adr + rgbgma_num0 * size, using_rgbgama, size);
	//res_read(p_sen_ident->rgb_gamma_tab_adr + rgbgma_num1 * size, using_rgbgama + size, size);
	memcpy(using_rgbgama,p_sen_ident->rgb_gamma_tab_adr + rgbgma_num0 * size,  size);
	memcpy(using_rgbgama + size,p_sen_ident->rgb_gamma_tab_adr + rgbgma_num1 * size, size);
	memcpy(isp.p_rgb_gama->using_rgbgama, using_rgbgama, size);
	memcpy((u32)isp.p_rgb_gama->using_rgbgama + size, using_rgbgama, size);
	if (!(data_check(isp.p_rgb_gama->using_rgbgama, 0, 32) && data_check(&isp.p_rgb_gama->using_rgbgama[511], 224, 255)))
	{
		printf("rgb gamma err\n");
		// printf("isp.p_rgb_gama->using_rgbgama resource_load err");
		// while(1);
	}
	return;
}
void isp_gma_table_load(u32 gma_num0, u32 gma_num1)
{
	...
	//res_read(p_sen_ident->ygamma_tab_adr + gma_num0 * size, using_ygama, size);
	//res_read(p_sen_ident->ygamma_tab_adr + gma_num1 * size, (u8)using_ygama + size, size);
	memcpy(using_ygama,p_sen_ident->ygamma_tab_adr + gma_num0 * size,size);
	memcpy(&using_ygama[256],p_sen_ident->ygamma_tab_adr + gma_num1 * size,size);

	memcpy(isp.p_ygama->using_ygama, using_ygama, size);
	memcpy((u32)isp.p_ygama->using_ygama + size, using_ygama, size);
	...
	{
		printf("y gamma err\n");
		// printf("isp.p_ygama->using_ygama resource_load err");
		// while(1);
	}
	return;
}
...
void isp_init(void)
{
	...
	csiirq_register_isp(isp_isr_hal, NULL);
...
{
	u32 i, dot_num = 0;
	for (i = 0; ((dot_num <= isp.p_rgb_gama->fog_dotnum) && (i <= 255)); i++)
	{ // 4000
		dot_num += (phgrm[i]);
	}
	*p_fog_y_low = i - 1;
...
bool soft_exp(STAT *pstat, u8 Br)
{
	...
	if ((_GET_ISP_MODE_(*isp.p_all_mod, _RGB_GAMA_POS_) == _ISP_AUTO_))
	{
		...
		if(Br>32)
			...
			fog_y_low=Br;
		isp.p_rgb_gama->rmin = isp.p_rgb_gama->gmin = isp.p_rgb_gama->bmin = fog_y_low;//clip((isp.p_rgb_gama->bmin * 5 + fog_y_low * 3) / 8, 0, isp.p_rgb_gama->fog_llimt);
		...
		if (((rgbgama_num[0] != rgbgama_temp[0]) || (rgbgama_num[1] != rgbgama_temp[1])) /*&& !CMOS_USB_ONLINE_DBG*/)
		{ //??????,??????
...
void isp_adjust(STAT *pstat, u32 *p_Br)
{
	...
	if (_GET_ISP_MODE_(*isp.p_all_mod, _CH_POS_) == _ISP_AUTO_)
	{
		...
		isp_CH_cfg(isp.p_ch->stage1_en, isp.p_ch->stage0_en, isp.p_ch->enhence, isp.p_ch->th1, isp.p_ch->th0, isp.p_ch->r_rate, isp.p_ch->g_rate, isp.p_ch->b_rate, ch_sat);
	}
	if (_GET_ISP_MODE_(*isp.p_all_mod, _VDE_POS_) == _ISP_AUTO_)
	{ // vde?????  BR
		u32 sat_rate, sat[9];
	...
	if (_GET_ISP_MODE_(*isp.p_all_mod, _EE_POS_) == _ISP_AUTO_)
	{ // ee ��?��?��?������|  BR
	...
	if (_GET_ISP_MODE_(*isp.p_all_mod, _DDC_POS_) == _ISP_AUTO_)
	{ // ddc ?????  br
...
void sensor_compensate_exp_set(u32 compensate_mod)
{
	...
	if (valve)
	{
		...
		{
		case 7:
			csiSensor->vde_adapt.bright_oft = 0x80;
			break;
		case 6:
			csiSensor->vde_adapt.bright_oft = 0x78;
			break;
		case 5:
			csiSensor->vde_adapt.bright_oft = 0x70;
			break;
		case 4:
			csiSensor->vde_adapt.bright_oft = 0x68;
			break;
		case 3:
			csiSensor->vde_adapt.bright_oft = 0x60;
			break;
		case 2:
			csiSensor->vde_adapt.bright_oft = 0x58;
			break;
		case 1:
			csiSensor->vde_adapt.bright_oft = 0x50;
			break;
		case 8:
			csiSensor->vde_adapt.bright_oft = 0x88;
			break;
		case 9:
			csiSensor->vde_adapt.bright_oft = 0x90;
			break;
		case 10:
			csiSensor->vde_adapt.bright_oft = 0x98;
			break;
		case 11:
			csiSensor->vde_adapt.bright_oft = 0xa0;
			break;
		case 12:
			csiSensor->vde_adapt.bright_oft = 0xa8;
			break;
		case 13:
			csiSensor->vde_adapt.bright_oft = 0xb0;
			break;				
		default:
			csiSensor->vde_adapt.bright_oft = 0x80;
			break;
		}
		printf("bright off = %d\n", isp.p_vde->bright_oft);
		isp_VDE_cfg(isp.p_vde->contra, isp.p_vde->bright_oft, isp.p_vde->bright_k, isp.p_vde->hue, isp.p_vde->sat);
	}
	else
	{
		if(compensate_mod>=1&&compensate_mod<=13)
			csiSensor->ae_adapt.exp_adapt.exp_ext_mod = compensate_mod;
	}
}

void sensor_sharpness_enhance(u8 coef)
{
	printf("[JRDEBUG] sensor_sharpness_enhance coef = %d\n", coef);
	_Sensor_Adpt_ *csiSensor = hal_csiAdptGet();
	u8 i = 0;
	switch (coef)
	{
	case 0:
	{
		for (i = 0; i < 8; i++)
		{
			csiSensor->ee_adapt.sharp_class[i] = 0x0;
		}
		break;
	}
	case 1:
	{
		for (i = 0; i < 8; i++)
		{
			csiSensor->ee_adapt.sharp_class[i] = 0x08;
		}
		break;
	}
	case 2:
	{
		for (i = 0; i < 8; i++)
		{
			csiSensor->ee_adapt.sharp_class[i] = 0x0f;
		}
		break;
	}
	default:
	{
		for (i = 0; i < 8; i++)
		{
			csiSensor->ee_adapt.sharp_class[i] = 0x02;
		}
		break;
	}
	}
...
/*******************************************************************************
 * Function Name  : color_on_off
 * Description    : enable color or disable color
 * Input          : on_off 0: disable color 1:enable color
 * Output         : none
 * Return         : none
 *******************************************************************************/
void isp_color_on_off(u8 on_off)
{
	/*
		static u32 saturation =0;
		if(saturation == 0)
		{
			saturation = isp.p_cbhs->s;
		}
		if(on_off)
		{
			isp.p_cbhs->s = saturation;
		}
		else
		{
			isp.p_cbhs->s = 0;
...
void sensor_frame_rate_adapt(u32 frequecy_mode, u32 frame_rate)
{
	...
	printf("sensor_frame_rate_adapt:%d\t%d\r\n", frequecy_mode, frame_rate);
	static int flag = 1;
	...
	if ((pointer_check(&(csiSensor->p_fun_adapt.fp_hvblank))) && flag)
	{

		printf("p_fun_adapt.fp_hvblank start\r\n");
		csiSensor->p_fun_adapt.fp_hvblank(h_len, csiSensor->hvb_adapt.v_len);
		printf("p_fun_adapt.fp_hvblank end\r\n");
		flag = 0;
	}
...
bool exposal_adj(u32 yloga_avg, u32 yloga, u32 *p_br, u32 exp_ext_mod)
{
	...
	*p_br = exp_br_cal(yloga, p_exp_adapt->exp_gain, p_exp_adapt->k_br);
	...
	ylog_obj = (ylog_obj > 28) ? (ylog_obj + exp_ext_mod * 4 - 28) : (exp_ext_mod * 4);
	...
	if (1)//(common_adapt.exp_gain_en)
	{
		p_exp_adapt->exp_gain = clip(p_exp_adapt->exp_gain, p_exp_adapt->exp_min * 256, p_exp_adapt->gain_max * step_val * step_max);
		if(common_adapt.exp_gain!=p_exp_adapt->exp_gain)
		{
			common_adapt.exp_gain=p_exp_adapt->exp_gain;
			...
			//printf("common  br:%d o:%d eg:%d e:%d g:%d avg:%d a:%d %d\n", *p_br, ylog_obj, p_exp_adapt->exp_gain, sen_exp, sen_gain, yloga_avg, yloga, common_adapt.exp_gain);
		}
	}
	else
	{
		...
		{																					   // ???????
			...
			//			}
			//	printf("auto   br:%d o:%d eg:%d e:%d g:%d avg:%d a:%d %d\n",*p_br,ylog_obj,p_exp_adapt->exp_gain,sen_exp,sen_gain,yloga_avg,yloga,common_adapt.exp_gain);
		}
	}
		...
		for (i = 0; i < _OL_DB_POS; i++)
		{
			if (_GET_ISP_MODE_(isp_module_state, i * _ISP_BIT_WIDTH_))
			{
				...
				case _BLC_POS_:
					...
					printf("usb BLC MODE:%d\n", val);
					break;
				case _LSC_POS_:
					isp_LSC_adj_cfg((u16 *)lsc_tab, 0);
					ax32xx_ispModeSet(ISP_MODE_LSCEN, val);
					printf("usb LSC MODE:%d\n", val);
					break;
				case _AWB_POS_:
					isp_AWB_adj_cfg(_GET_ISP_MODE_(*isp.p_all_mod, _AWB_POS_), (u8 *)isp.p_awb->awb_tab, isp.p_awb->seg_mode, isp.p_awb->rgmin, isp.p_awb->rgmax,
									isp.p_awb->weight_in, isp.p_awb->weight_mid, isp.p_awb->ymin, isp.p_awb->ymax, isp.p_awb->hb_rate, isp.p_awb->hb_class, isp.p_awb->hr_rate, isp.p_awb->hr_class,
					...
					printf("usb AWB MODE:%d\n", val);
					break;
				case _YGAMA_POS_:
					isp_GAMMA_cfg(isp.p_ygama->using_ygama, isp.p_ygama->br_mod, using_contrast, isp.p_ygama->bofst, isp.p_ygama->lofst, NULL, NULL, isp.p_ygama->pad_num);
					ax32xx_ispModeSet(ISP_MODE_YGAMMAEN, val);
					printf("usb YGAMA MODE:%d\n", val);
					break;
				case _DDC_POS_:
					isp_DDC_cfg(isp.p_ddc->dpc_dn_en, isp.p_ddc->hot_num, isp.p_ddc->dead_num, isp.p_ddc->hot_th, isp.p_ddc->dead_th, isp.p_ddc->avg_th,
								isp.p_ddc->d_th_rate[7], isp.p_ddc->h_th_rate[7], isp.p_ddc->std_th, isp.p_ddc->std_th_rate, isp.p_ddc->indx_table);
					ax32xx_ispModeSet(ISP_MODE_DPCEN, val);
					printf("DDC MODE:%d\n", val);
					break;

				case _CCM_POS_:
					isp_CCM_cfg(isp.p_ccm->ccm, isp.p_ccm->s41, isp.p_ccm->s42, isp.p_ccm->s43);
					ax32xx_ispModeSet(ISP_MODE_CCMEN, val);
					printf("CCM MODE:%d\n", val);
					break;

				case _DGAIN_POS_:
					isp_RGB_DGAIN_adj(isp.p_rgb_dgain->dgain[0], isp.p_rgb_dgain->dgain[1], isp.p_rgb_dgain->dgain[2], isp.p_rgb_dgain->dgain[3], isp.p_rgb_dgain->dgain[4],
									  isp.p_rgb_dgain->dgain[5], isp.p_rgb_dgain->dgain[6], isp.p_rgb_dgain->dgain[7], isp.p_rgb_dgain->dgain[8]);
					ax32xx_ispModeSet(ISP_MODE_DGAINEN, val);
					printf("DEGAIN MODE:%d\n", val);
					break;

				case _RGB_GAMA_POS_:
					isp_RGB_gam_cfg(isp.p_rgb_gama->using_rgbgama, isp.p_rgb_gama->br_mod, isp.p_rgb_gama->max_oft[7], isp.p_rgb_gama->rmin, isp.p_rgb_gama->rmax, isp.p_rgb_gama->gmin, isp.p_rgb_gama->gmax, isp.p_rgb_gama->bmin, isp.p_rgb_gama->bmax);
					ax32xx_ispModeSet(ISP_MODE_RGBGAMMAEN, val);
					printf("RGB_GAMA MODE:%d\n", val);
					break;

				case _CH_POS_:
					isp_CH_cfg(isp.p_ch->stage1_en, isp.p_ch->stage0_en, isp.p_ch->enhence, isp.p_ch->th1, isp.p_ch->th0, isp.p_ch->r_rate, isp.p_ch->g_rate, isp.p_ch->b_rate, isp.p_ch->sat);
					ax32xx_ispModeSet(ISP_MODE_CHEN, val);
					printf("CH MODE:%d\n", val);
					break;

				case _VDE_POS_:
					isp_VDE_cfg(isp.p_vde->contra, isp.p_vde->bright_oft, isp.p_vde->bright_k, isp.p_vde->hue, isp.p_vde->sat);
					ax32xx_ispModeSet(ISP_MODE_VDEEN, val);
					printf("VDE MODE:%d\n", val);
					break;

				case _EE_POS_:
					isp_EE_cfg(isp.p_ee->ee_dn_th[7], isp.p_ee->ee_dn_slope[7], isp.p_ee->ee_sharp_slope[7], isp.p_ee->ee_th_adp[7], isp.p_ee->sharp_class[7], isp.p_ee->dn_class[7]);
					ax32xx_ispModeSet(ISP_MODE_SHARPEN, val);
					printf("EE MODE:%d &x\n", val);
					break;

				case _CFD_POS_:
					isp_CCF_cfg(isp.p_ccf->ccf_en, isp.p_ccf->wdc_en, isp.p_ccf->wclass, isp.p_ccf->rate, isp.p_ccf->wymin, isp.p_ccf->ymax, isp.p_ccf->mean_en, isp.p_ccf->th, isp.p_ccf->dn_class);
					ax32xx_ispModeSet(ISP_MODE_CCFEN, val);
					printf("CFD MODE:%d\n", val);
					break;
				...
				case _SAJ_POS_:
					isp_SAJ_cfg(isp.p_saj->sat[0], isp.p_saj->sat[1], isp.p_saj->sat[2], isp.p_saj->sat[3], isp.p_saj->sat[4], isp.p_saj->sat[5], isp.p_saj->sat[6], isp.p_saj->sat[7], isp.p_saj->sat[8],
								isp.p_saj->sat[9], isp.p_saj->sat[10], isp.p_saj->sat[11], isp.p_saj->sat[12], isp.p_saj->sat[13], isp.p_saj->sat[14], isp.p_saj->sat[15], isp.p_saj->sat[16]);
					ax32xx_ispModeSet(ISP_MODE_SAJEN, val);
					printf("SAJ MODE:%d\n", val);
					break;
...
{ //???????????,????????????????
	...
	if (_GET_BIT_(commom_mode, common_mode_max))
	{
		...
		for (i = 0; i < common_mode_max; i++)
		{
			...
			switch (i)
			{
			case pclk_fir_en_pos:
				ax32xx_pclk_fir_Set(common_adapt.pclk_fir_en);
				break;
			case pclk_fir_class_pos:
				break;
			case pclk_inv_en_pos:
				ax32xx_pclk_inv_Set(common_adapt.pclk_inv_en);
				break;
			case csi_tun_pos:
				ax32xx_csi_clk_tun_Set(common_adapt.csi_tun);
				break;
			case hsyn_pos:
				ax32xx_csiSyncSet(common_adapt.hsyn, common_adapt.vsyn);
				break;
			case vsyn_pos:
				ax32xx_csiSyncSet(common_adapt.hsyn, common_adapt.vsyn);
				break;
			case colrarray_pos:
				ax32xx_csiPrioritySet(common_adapt.colrarray);
				break;
			case rduline_pos:
				break;
			case v_len_pos:
				break;
			case down_fps_mode_pos:
				break;
			case fps_pos:
			...
			case mclk_pos:
				break;
			case pclk_pos:
				printf("no suppot change pclk yet\n");
				break;
			case rotate_pos:
				csiSensor->p_fun_adapt.fp_rotate(common_adapt.rotate);
				break;
			case AVDD_pos:
				ax32xx_sysLDOSet(SYS_LDO_HSEN, common_adapt.AVDD, 1);
				break;
			case DVDD_pos:
				ax32xx_sysLDOSet(SYS_LDO_LSEN, common_adapt.DVDD, 1);
				break;
			case VDDIO_pos:
				printf("the board didn't support set VDDIO:%d\n", common_adapt.VDDIO);
				break;
				/*
			   case pclk_fir_en_pos:		printf("fir_en:%d\n",common_adapt.pclk_fir_en);	break;
			   case pclk_fir_class_pos:	printf("fir_class:%d\n",common_adapt.pclk_fir_class);break;
			   case pclk_inv_en_pos:		printf("inv_en:%d\n",common_adapt.pclk_inv_en);break;
			   case csi_tun_pos:			printf("tun:%d\n",common_adapt.csi_tun);break;
			   case hsyn_pos:				printf("hsyn:%d\n",common_adapt.hsyn);break;
			   case vsyn_pos:				printf("vsyn:%d\n",common_adapt.vsyn);break;
			   case colrarray_pos:			printf("array:%d\n",common_adapt.colrarray);break;
			   case rduline_pos:			printf("rduline:%d\n",common_adapt.rduline);break;
			   case v_len_pos:				printf("v_len:%d\n",common_adapt.hvb.v_len);break;
			   case down_fps_mode_pos:		printf("down:%d\n",common_adapt.hvb.down_fps_mode);break;
			   case fps_pos:				printf("fps:%d\n",common_adapt.hvb.fps);break;
			   case frequency_pos:			printf("fre:%d\n",common_adapt.hvb.frequency);break;
			   case mclk_pos:				printf("mclk:%d\n",common_adapt.mclk);break;
			   case pclk_pos:				printf("no suppot change pclk yet:%d\n",common_adapt.hvb.pclk);break;
			   case rotate_pos:			printf("rotate:%d\n",common_adapt.rotate);break;
			   case AVDD_pos:				printf("avdd:%d\n",common_adapt.AVDD);break;
			   case DVDD_pos:				printf("dvdd:%d\n",common_adapt.DVDD);break;
			   case VDDIO_pos:				printf("the board didn't support set VDDIO:%d\n",common_adapt.VDDIO);break;*/
			default:
				break; //
			}
		}
...
void hal_isp_debug_read(u8 *pbdata, u32 trans_len, u32 param_3B)
{
	...
	case _DDC_POS_:
		...
		printf("%d %d %d %d %d %d %d %d\n", isp.p_ddc->indx_adapt[0], isp.p_ddc->indx_adapt[1], isp.p_ddc->indx_adapt[2], isp.p_ddc->indx_adapt[3], isp.p_ddc->indx_adapt[4], isp.p_ddc->indx_adapt[5], isp.p_ddc->indx_adapt[6], isp.p_ddc->indx_adapt[7]);
	...
	case _AWB_POS_:
		...
		break;
	case _CCM_POS_:
		memcpy(pbdata, (char *)isp.p_ccm + startPos, trans_len);
	...
	case _YGAMA_POS_:
		...
		break;
	case _RGB_GAMA_POS_:
		memcpy(pbdata, (char *)isp.p_rgb_gama + startPos, trans_len);
		// uart_PrintfBuf(pbdata,sizeof(devSensor_Cmd.rgbgama_adapt));
		break;
	case _EE_POS_:
		memcpy(pbdata, (char *)isp.p_ee + startPos, trans_len);
		// uart_PrintfBuf(pbdata,sizeof(devSensor_Cmd.ee_adapt));
		break;
	case _LSC_POS_:
		memcpy(pbdata, (char *)lsc_tab + startPos, trans_len);
		printf("len:%d 3B:%x\n", trans_len, param_3B);
		printf("lsc_tab:\n");
		printf("startPos=%d\n", startPos);
		printf("size:%d\n", trans_len);
...
void hal_isp_debug_write(u8 *pbdata, u32 trans_len, u32 param_3B)
{
	...
	case _AE_POS_:
		...
		printf("%d %d %d %d %d %d %d %d\n", isp.p_ae->exp_adapt.exp_tag[0], isp.p_ae->exp_adapt.exp_tag[1], isp.p_ae->exp_adapt.exp_tag[2], isp.p_ae->exp_adapt.exp_tag[3], isp.p_ae->exp_adapt.exp_tag[4], isp.p_ae->exp_adapt.exp_tag[5], isp.p_ae->exp_adapt.exp_tag[6], isp.p_ae->exp_adapt.exp_tag[7]);
		printf("%d\n", isp.p_ae->exp_adapt.gain_max);
		break;
	case _BLC_POS_:
		printf("wirte blc\n");
	...
	case _CCM_POS_:
		...
		printf("%d %d %d %d %d %d %d %d %d\n", isp.p_ccm->ccm[0], isp.p_ccm->ccm[1], isp.p_ccm->ccm[2], isp.p_ccm->ccm[3], isp.p_ccm->ccm[4], isp.p_ccm->ccm[5], isp.p_ccm->ccm[6], isp.p_ccm->ccm[7], isp.p_ccm->ccm[8]);
	...
	case _CH_POS_:
		...
		printf("%d %d %d %d %d %d %d\n", isp.p_ch->rate[0], isp.p_ch->rate[1], isp.p_ch->rate[2], isp.p_ch->rate[3], isp.p_ch->rate[4], isp.p_ch->rate[5], isp.p_ch->rate[6], isp.p_ch->rate[7]);
		...
		printf("%d %d %d %d %d %d\n", isp.p_ch->enhence[0], isp.p_ch->enhence[1], isp.p_ch->enhence[2], isp.p_ch->enhence[3], isp.p_ch->enhence[4], isp.p_ch->enhence[5]);
	...
	case _EE_POS_:
		...
		for (i = 0; i < 8; i++)
		{
			printf("%d ", isp.p_ee->sharp_class[i]);
		}
		printf("\n");
	...
	case _VDE_POS_:
		...
		printf("%d %d %d %d %d %d %d %d\n", isp.p_vde->sat_rate[0], isp.p_vde->sat_rate[1], isp.p_vde->sat_rate[2], isp.p_vde->sat_rate[3], isp.p_vde->sat_rate[4], isp.p_vde->sat_rate[5], isp.p_vde->sat_rate[6], isp.p_vde->sat_rate[7]);
	...
	case _SAJ_POS_:
		...
		printf("%d %d %d %d %d %d %d %d\n", isp.p_saj->sat_rate[0], isp.p_saj->sat_rate[1], isp.p_saj->sat_rate[2], isp.p_saj->sat_rate[3], isp.p_saj->sat_rate[4], isp.p_saj->sat_rate[5], isp.p_saj->sat_rate[6], isp.p_saj->sat_rate[7]);
...
void hal_Sensor_common_Set(u8 *pbdata, u32 trans_len, u32 param_3B)
{
	...
	switch (startPos)
	{
	case offsetof(COMMON, exp_gain):
		CopyToMember(&common_adapt, offsetof(COMMON, exp_gain),
					 sizeof(common_adapt.exp_gain), pbdata, startPos, trans_len);
		break;
	case offsetof(COMMON, gain_max):
		CopyToMember(&common_adapt, offsetof(COMMON, gain_max),
					 sizeof(common_adapt.gain_max), pbdata, startPos, trans_len);
		_Sensor_Adpt_ *csiSensor = hal_csiAdptGet();
		csiSensor->ae_adapt.exp_adapt.gain_max = common_adapt.gain_max;
		break;
	case offsetof(COMMON, mclk):
		CopyToMember(&common_adapt, offsetof(COMMON, mclk),
	...
	case offsetof(COMMON, hsyn):
		CopyToMember(&common_adapt, offsetof(COMMON, hsyn),
					 sizeof(common_adapt.hsyn), pbdata, startPos, trans_len);
		_SET_BIT_(commom_mode, hsyn_pos, 1);
		_SET_BIT_(commom_mode, common_mode_max, 1);
		break;
	case offsetof(COMMON, vsyn):
		CopyToMember(&common_adapt, offsetof(COMMON, vsyn),
					 sizeof(common_adapt.vsyn), pbdata, startPos, trans_len);
		_SET_BIT_(commom_mode, vsyn_pos, 1);
		_SET_BIT_(commom_mode, common_mode_max, 1);
		break;
	case offsetof(COMMON, rduline):
		CopyToMember(&common_adapt, offsetof(COMMON, rduline),
					 sizeof(common_adapt.rduline), pbdata, startPos, trans_len);
		_SET_BIT_(commom_mode, rduline_pos, 1);
		_SET_BIT_(commom_mode, common_mode_max, 1);
		break;
	case offsetof(COMMON, colrarray):
		CopyToMember(&common_adapt, offsetof(COMMON, colrarray),
					 sizeof(common_adapt.colrarray), pbdata, startPos, trans_len);
		_SET_BIT_(commom_mode, colrarray_pos, 1);
		_SET_BIT_(commom_mode, common_mode_max, 1);
		break;
	case offsetof(COMMON, pclk_fir_en):
		CopyToMember(&common_adapt, offsetof(COMMON, pclk_fir_en),
					 sizeof(common_adapt.pclk_fir_en), pbdata, startPos, trans_len);
		_SET_BIT_(commom_mode, pclk_fir_en_pos, 1);
		_SET_BIT_(commom_mode, common_mode_max, 1);
		break;
	case offsetof(COMMON, pclk_inv_en):
		CopyToMember(&common_adapt, offsetof(COMMON, pclk_inv_en),
					 sizeof(common_adapt.pclk_inv_en), pbdata, startPos, trans_len);
		_SET_BIT_(commom_mode, pclk_inv_en_pos, 1);
		_SET_BIT_(commom_mode, common_mode_max, 1);
		break;
	case offsetof(COMMON, csi_tun):
		CopyToMember(&common_adapt, offsetof(COMMON, csi_tun),
					 sizeof(common_adapt.csi_tun), pbdata, startPos, trans_len);
		_SET_BIT_(commom_mode, csi_tun_pos, 1);
		_SET_BIT_(commom_mode, common_mode_max, 1);
		break;
	case offsetof(COMMON, exp_gain_en):
		CopyToMember(&common_adapt, offsetof(COMMON, exp_gain_en),
					 sizeof(common_adapt.exp_gain_en), pbdata, startPos, trans_len);
		break;
	case offsetof(COMMON, blk_en):
		CopyToMember(&common_adapt, offsetof(COMMON, blk_en),
		...
		break;
	case offsetof(COMMON, lsc_en):
		CopyToMember(&common_adapt, offsetof(COMMON, lsc_en),
		...
		break;
	case offsetof(COMMON, ddc_en):
		CopyToMember(&common_adapt, offsetof(COMMON, ddc_en),
		...
		break;
	case offsetof(COMMON, awb_en):
		CopyToMember(&common_adapt, offsetof(COMMON, awb_en),
		...
		break;
	case offsetof(COMMON, ccm_en):
		CopyToMember(&common_adapt, offsetof(COMMON, ccm_en),
		...
		break;
	case offsetof(COMMON, dgain_en):
		CopyToMember(&common_adapt, offsetof(COMMON, dgain_en),
		...
		break;
	case offsetof(COMMON, ygama_en):
		CopyToMember(&common_adapt, offsetof(COMMON, ygama_en),
		...
		break;
	case offsetof(COMMON, rgb_gama_en):
		CopyToMember(&common_adapt, offsetof(COMMON, rgb_gama_en),
		...
		break;
	case offsetof(COMMON, ch_en):
		CopyToMember(&common_adapt, offsetof(COMMON, ch_en),
		...
		break;
	case offsetof(COMMON, vde_en):
		CopyToMember(&common_adapt, offsetof(COMMON, vde_en),
		...
		break;
	case offsetof(COMMON, ee_en):
		CopyToMember(&common_adapt, offsetof(COMMON, ee_en),
		...
		break;
	case offsetof(COMMON, cfd_en):
		CopyToMember(&common_adapt, offsetof(COMMON, cfd_en),
		...
		break;
	case offsetof(COMMON, saj_en):
		CopyToMember(&common_adapt, offsetof(COMMON, saj_en),
		...
		break;
	case offsetof(COMMON, AVDD):
		CopyToMember(&common_adapt, offsetof(COMMON, AVDD),
					 sizeof(common_adapt.AVDD), pbdata, startPos, trans_len);
		_SET_BIT_(commom_mode, AVDD_pos, 1);
		_SET_BIT_(commom_mode, common_mode_max, 1);
		break;
	case offsetof(COMMON, DVDD):
		CopyToMember(&common_adapt, offsetof(COMMON, DVDD),
					 sizeof(common_adapt.DVDD), pbdata, startPos, trans_len);
		_SET_BIT_(commom_mode, DVDD_pos, 1);
		_SET_BIT_(commom_mode, common_mode_max, 1);
		break;
	case offsetof(COMMON, VDDIO):
		CopyToMember(&common_adapt, offsetof(COMMON, VDDIO),
					 sizeof(common_adapt.VDDIO), pbdata, startPos, trans_len);
		_SET_BIT_(commom_mode, VDDIO_pos, 1);
		_SET_BIT_(commom_mode, common_mode_max, 1);
		break;
	case offsetof(COMMON, pclk_fir_class):
		CopyToMember(&common_adapt, offsetof(COMMON, pclk_fir_class),
					 sizeof(common_adapt.pclk_fir_class), pbdata, startPos, trans_len);
		_SET_BIT_(commom_mode, pclk_fir_en_pos, 1);
		_SET_BIT_(commom_mode, common_mode_max, 1);
		break;
	case offsetof(COMMON, rotate):
		CopyToMember(&common_adapt, offsetof(COMMON, rotate),
	...
	default:
		printf("do not have this module\n");
		break;
	}
	...
	printf("isp_module_state:%x isp.p_all_mod:%x\n", isp_module_state, *isp.p_all_mod);
	//	printf("blk_en:%d\n",common_adapt.blk_en);
	//	printf("lsc_en:%d\n",common_adapt.lsc_en);
	//	printf("ddc_en:%d\n",common_adapt.ddc_en);
	//	printf("awb_en:%d\n",common_adapt.awb_en);
	...
	//	printf("saj_en:%d\n",common_adapt.saj_en);
	//	printf("vddio:%d\n",common_adapt.VDDIO);
	//	printf("exp_gain_en:%d\n",common_adapt.exp_gain_en);
	//	printf("exp_gain:%d\n",common_adapt.exp_gain);
}

static struct timer_list isp_timer;
void isp_init_timertask()
{
#if 0
	isp_timer.function = isp_process_check;
	isp_timer.data = &isp;
	osal_init_timer(&isp_timer);
	osal_auto_timer(&isp_timer, 15);
#endif
}
...
void CopyToMember(void *ispModuleData, int offsetOfMember, int sizeOfMember, void *pbdata, int startPos, int trans_len)
{
	...
	else if (startPos > offsetOfMember && startPos <= offsetOfMember + sizeOfMember)
	{
		memcpy((char *)ispModuleData + startPos,
			   pbdata,
			   endPos > offsetOfMember + sizeOfMember ? sizeOfMember : endPos - startPos);
	}
}
