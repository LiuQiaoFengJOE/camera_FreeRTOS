...
#define _SET_ISP_MODE_(x, model, val)                       \
	do                                                      \
	{                                                       \
		x = ((x) & (~(0x03 << (model))));                   \
		x = ((x) | (((val) & (_ISP_MODE_BW_)) << (model))); \
	} while (0);
#define _GET_ISP_MODE_(x, model) (((x) >> model) & _ISP_MODE_BW_)

#define _SET_BIT_(x, model, val)                  \
	do                                            \
	{                                             \
		x = ((x) & (~(1 << (model))));            \
		x = ((x) | (((val) & (0x1)) << (model))); \
...
#define _ISP_MODE_DEFAULT_ (_ISP_FREE_ << _AE_POS_ | _ISP_EN_ << _BLC_POS_ | _ISP_EN_ << _LSC_POS_ | _ISP_AUTO_ << _DDC_POS_ | _ISP_AUTO_ << _AWB_POS_ | _ISP_DIS_ << _CCM_POS_ | _ISP_AUTO_ << _DGAIN_POS_ | _ISP_AUTO_ << _YGAMA_POS_ | _ISP_AUTO_ << _RGB_GAMA_POS_ | _ISP_DIS_ << _CH_POS_ | _ISP_DIS_ << _VDE_POS_ | _ISP_AUTO_ << _EE_POS_ | _ISP_DIS_ << _CFD_POS_ | _ISP_DIS_ << _SAJ_POS_)
...
typedef struct
{
	u32 hot_num;  // 5*5?D??????24-hot_num???????a???hot_point
	u32 dead_num; // 5*5?D??????24-dead_num???????a???hot_point
	u32 hot_th;
	u32 dead_th;
	u32 avg_th;
	u32 d_th_rate[8];
...
extern u8 gama[_RGBGAMA_CLSASSES_][256];		 // __attribute__ ((section(".sensor_res.isp_tab"))) ;
extern u8 color_compress[_YGAMA_CLSASSES_][256]; // __attribute__ ((section(".sensor_res.isp_tab"))) ;
extern u8 contra_tab[_CONTRA_CLSASSES_][256];	 // __attribute__ ((section(".sensor_res.isp_tab"))) ;

extern u16 using_ygama[512];
extern u8 using_rgbgama[512];
