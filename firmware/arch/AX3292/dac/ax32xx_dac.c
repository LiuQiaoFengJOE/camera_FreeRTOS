...
//-------------dac clk----------------------
    ...
    R_DAC_CTRL0 |= (1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | (1 << 9) | (1 << 10) | (1 << 13) | (1 << 19) | (1 << 20) | (1 << 22) | (1 << 24);
    R_DAC_CTRL1 |= (1 << 2) | (1 << 5);
...
typedef enum
{
    DAC_IRQ_EMPTY = 0, // dac buffer空
    DAC_IRQ_HALF,      // buffer半空
    DAC_IRQ_PEND,
    DAC_IRQ_MAX
} DAC_IRQ_E;
...
isr_func dac_vector_table[DAC_IRQ_MAX];
void *dac_dev_table[DAC_IRQ_MAX];

void dac_irq(unsigned int irqno, void *d, void *regs);
void dacirq_register(uint32_t irq, isr_func isr, void *dev_id)
{
    irq_register(IRQNO_DAC, 0, dac_irq, NULL);
    dac_vector_table[irq] = isr;
    dac_dev_table[irq] = dev_id;
    // CSI_INTEN |= BIT(irq) | 0x80000000;
    printf("\xA1\xBE%s\xA1\xBF%s %d %X\r\n", pcTaskGetName(NULL), __FUNCTION__, IRQNO_DAC, isr);
}

void dac_register_half(isr_func isr, void *dev_id)
{
    dacirq_register(DAC_IRQ_HALF, isr, dev_id);
}

void dac_register_empty(isr_func isr, void *dev_id)
{
    dacirq_register(DAC_IRQ_EMPTY, isr, dev_id);
}

void dac_register_INT(isr_func isr, void *dev_id)
{
    dacirq_register(DAC_IRQ_PEND, isr, dev_id);
}

// 中断的时候会调用这个函数，接着在里面判断
void dac_irq(unsigned int irqno, void *d, void *regs)
{
    // printf("dac_irq enter\n");

    // INT PEND
    if (R_DAC_OSTAUS & BIT(0))
    {
        R_DAC_OSYNC |= BIT(0); // clear all flag
                               //		R_DAC_OSYNC |= BIT(0); // clear all flag
                               //		R_DAC_OSYNC |= BIT(2);
        if (dac_vector_table[DAC_IRQ_PEND])
            dac_vector_table[DAC_IRQ_PEND](DAC_IRQ_PEND, dac_dev_table[DAC_IRQ_PEND], regs);
        // printf("4\n");
    }

    // half
    else if (R_DAC_OSTAUS & BIT(1))
    {
        R_DAC_OSYNC |= BIT(1); // clear half
        if (dac_vector_table[DAC_IRQ_HALF])
            dac_vector_table[DAC_IRQ_HALF](DAC_IRQ_HALF, dac_dev_table[DAC_IRQ_HALF], regs);
    }

    // empty
    else if (R_DAC_OSTAUS & BIT(2))
    {
        // R_DAC_OSYNC |= BIT(0); // clear all flag
        R_DAC_OSYNC |= BIT(2);
        if (dac_vector_table[DAC_IRQ_EMPTY])
            dac_vector_table[DAC_IRQ_EMPTY](DAC_IRQ_EMPTY, dac_dev_table[DAC_IRQ_EMPTY], regs);
    }
}
