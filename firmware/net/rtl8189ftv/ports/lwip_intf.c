 ...
 *      Return Value: None
 */
void rltk_wlan_send(int idx, struct eth_drv_sg *sg_list, int sg_len, int total_len)
{
...
{
	return (uint8_t *)&(xnetif[idx].ip_addr);
}
#endif
...
{
	if (sys_arch_timeout_exist(arp_timer))
	{
...
////////////////////////////////////////////////////////////////////////////////
/* sys_arch.c中的timeoutlist貌似是沒有用的東西
 * 所以上面lwip_PRE_SLEEP_PROCESSING之類也是沒有用的東西
 * 因此下面這兩個函數也是沒有用的東西
 * 估計是上古時期lwip的timeout沒有弄好，所以才需要應用搞個自己的timeout列表
 * 如果上述猜測屬實，則下面兩個函數置空即可
 */
