...
int wext_private_command(const char *ifname, char *cmd, int show_msg)
{
	...
	if (strcmp((const char *)cmdname, "read_mac") == 0)
		strcpy(cmd, buf);
	vPortFree(buf);
	return ret;
}
...
void wext_wlan_indicate(unsigned int cmd, union iwreq_data *wrqu, char *extra)
{
	...
	memset(null_mac,0x0,sizeof(null_mac));
	...
	case IWEVCUSTOM:
		if (extra)
		{
			printf("Here in wext_wlan_indicate(IWEVCUSTOM),extra:%s\n",extra);
			...
			{
				printf("Start wifi_indication!\n");
				mac_add(wrqu->addr.sa_data);
				wifi_indication(WIFI_EVENT_STA_DISASSOC, wrqu->addr.sa_data, sizeof(null_mac), 0);
				deleteMacRtspConn();
				printf("Stop wifi_indication!\n");
			}
