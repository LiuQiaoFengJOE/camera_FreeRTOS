...
static void wifi_connected_hdl(char *buf, int buf_len, int flags, void *userdata)
{
	if ((join_user_data != NULL) && ((join_user_data->network_info.security_type == RTW_SECURITY_OPEN) ||
									 ...
									 (join_user_data->network_info.security_type == RTW_SECURITY_WPA2_AES_PSK)))
	{
	...
	void *semaphore)
{
	...
	printf("\r\nwps_wifi.ssid = %s\n", join_result->network_info.ssid.val);
	printf("\r\nsecurity_type = %d\n", join_result->network_info.security_type);
	printf("\r\nwps_wifi.password = %s\n", join_result->network_info.password);
	printf("\r\nssid_len = %d\n", join_result->network_info.ssid.len);
	printf("\r\npassword_len = %d\n", join_result->network_info.password_len);
...
{
	return wext_get_txpower(WLAN0_NAME, poweridx);
}

int wifi_set_txpower(int poweridx)
{
	return wext_set_txpower(WLAN0_NAME, poweridx);
}
...
int wifi_set_country(rtw_country_code_t country_code)
{
	...
	{
	case RTW_COUNTRY_US:
		ret = wext_set_country(WLAN0_NAME, "US");
		break;
	case RTW_COUNTRY_EU:
		ret = wext_set_country(WLAN0_NAME, "EU");
		break;
	case RTW_COUNTRY_JP:
		ret = wext_set_country(WLAN0_NAME, "JP");
		break;
	case RTW_COUNTRY_CN:
		ret = wext_set_country(WLAN0_NAME, "CN");
		break;
	default:
...
int wifi_on(rtw_mode_t mode)
{
	...
	printf("rltk_wlan_init end\r\n");

	for (idx = 0; idx < devnum; idx++)
	{
		printf("rltk_wlan_starting %d\r\n", idx);
		rltk_wlan_start(idx);
	}
	printf("rltk_wlan_start end\r\n");
