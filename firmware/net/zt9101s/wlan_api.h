...
struct wlan_app_info_t
{
    ...
    int16_t channel;
    /* 2.4G/5G */
    uint32_t band; //0 = 5G, 1 = 2.4G, 0x7fffffff = unknown
    /* maximal data rate */
    uint32_t datarate;
    /* signal strength */
    int8_t rssi;
 ...
 * @return 0:succ; others:fail
 */
int wlan_app_ap_start(const char *name, uint16_t channel, const char *ssid, const char *passwd);
 ...
 * @return 0:succ; others:fail
 */
int wlan_get_mac_address(const char *ifname, uint8_t* mac_addr);
