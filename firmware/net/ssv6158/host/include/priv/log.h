...
#define LOG_DEBUGF(debug, message) do { \
                                    if ( \
                                   ((debug) & g_log_module) && \
                                   ((ssv_type_s16)((debug) & LOG_LEVEL_MASK_LEVEL) >= (ssv_type_s16)g_log_min_level)) { \
...
extern ssv_type_u32					g_dbg_mutex;
//if (!gOsFromISR)
...
typedef struct
{
	...
	ssv_type_u32     prn_tag_lvl;
	ssv_type_u32     prn_tag_mod;
	ssv_type_u32     prn_tag_sprs;
	ssv_type_u32     chk_tag_sprs;	// won't check 'level' & 'module', ONLY used by log_printf().
...
typedef struct
{
	ssv_type_u8		dst_open;			// opened  destination
	ssv_type_u8		dst_cur;			// current destination
	void	*fp;				// the log file ptr  on host side
	ssv_type_u8		path[LOG_MAX_PATH];	// the log file path on host side
} log_out_desc_st;
...
extern ssv_type_bool	LOG_out_dst_open(ssv_type_u8 _dst, const ssv_type_u8 *_path);
extern ssv_type_bool	LOG_out_dst_close(ssv_type_u8 _dst);
extern ssv_type_bool	LOG_out_dst_turn_on(ssv_type_u8 _dst);
extern ssv_type_bool	LOG_out_dst_turn_off(ssv_type_u8 _dst);
extern void LOG_out_desc_dump(void);
extern ssv_type_bool LOG_out_init(void);
#endif
...
extern void ssv6xxx_printf(ssv_type_u32 n, ssv_type_u32 m, const char *file, ssv_type_u32 line, const char *fmt, ...);
extern void ssv6xxx_fatal(ssv_type_u32 m, const char *file, ssv_type_u32 line, const char *fmt, ...);
extern int ssv6xxx_vsnprintf(char *out, ssv_type_size_t size, const char *format, va_list args);
...
#define LOG_EVT_PRNF_BUF_MAX	(LOG_DMSG_MAX_ARGS*sizeof(ssv_type_u32) + 32)	// 32 -> rsvd space for safety
#define LOG_EVT_PRNF_MAXLEN		(sizeof(log_evt_prnf_st) + LOG_EVT_PRNF_BUF_MAX)

// ===============================================================================================
//											Misc
// ===============================================================================================
extern void LOG_init(ssv_type_bool tag_level, ssv_type_bool tag_mod, ssv_type_u32 level, ssv_type_u32 mod_mask, ssv_type_bool fileline);
