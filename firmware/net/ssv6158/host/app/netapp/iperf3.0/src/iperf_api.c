...
extern ssv_type_u32 unit_atoi_rate(const char *s);
...
void *iperf_malloc(ssv_type_size_t size)
{
#ifdef IPERF_DEBUG_MEM
    ssv_type_u8 * ptr;
    ptr = (ssv_type_u8 *)OS_MemAlloc(size+4);
    if (ptr) {
        iperf_mallocs++;
        iperf_msize += size;
        *(int*)ptr = size;
        ...
        //LOG_PRINTF("iperf_malloc size: %u\r\n", size);
        ptr += 4;
    }    
    return (void*)ptr;
#else
    return (void*)OS_MemAlloc(size);
#endif    
}
...
#ifdef IPERF_DEBUG_MEM
    if (ptr){
        ptr = (ssv_type_u8 *)ptr - 4;
        iperf_frees++;
        iperf_msize -= *(int*)ptr;
        //LOG_PRINTF("iperf_free size: %u\r\n", *(int*)ptr);
        OS_MemFree(ptr);
    }
#else
    if (ptr)
...
iperf_usage(void)
{
    LOG_PRINTF("Usage: iperf [-s|-c host] [options]\r\n");
    LOG_PRINTF("Try `iperf --help' for more information.\r\n");
}

void
usage_long(void)
{
    LOG_PRINTF("Usage: iperf3 [-s|-c host] [options]\r\n");
    LOG_PRINTF("       iperf3 [-h|--help] [-v|--version]\r\n\r\n");
    LOG_PRINTF("Server or Client:\r\n");
    LOG_PRINTF("  -p, --port      #         server port to listen on/connect to\r\n");
    LOG_PRINTF("  -i, --interval  #         seconds between periodic bandwidth reports\r\n");
    LOG_PRINTF("  -V, --verbose             more detailed output\r\n");
    LOG_PRINTF("  -d, --debug               emit debugging output\r\n");
    LOG_PRINTF("  -v, --version             show version information and quit\r\n");
    LOG_PRINTF("  -h, --help                show this message and quit\r\n");
    LOG_PRINTF("  stop                      kill the iperf threads \r\n");
    LOG_PRINTF("  list                      show the running iperf threads \r\n");
    LOG_PRINTF("Server specific:\r\n");
    LOG_PRINTF("  -s, --server              run in server mode\r\n");
    LOG_PRINTF("Client specific:\r\n");
    LOG_PRINTF("  -c, --client    <host>    run in client mode, connecting to <host>\r\n");
    LOG_PRINTF("  -u, --udp                 use UDP rather than TCP\r\n");
    LOG_PRINTF("  -b, --bandwidth #[KMG][/#] target bandwidth in bits/sec (0 for unlimited)\r\n");
    LOG_PRINTF("                            (default %d Mbit/sec for UDP, unlimited for TCP)\r\n", UDP_RATE / (1024*1024));
    LOG_PRINTF("                            (optional slash and packet count for burst mode)\r\n");
    LOG_PRINTF("  -t, --time      #         time in seconds to transmit for (default %d secs)\r\n", DURATION);
    LOG_PRINTF("  -n, --bytes     #[KMG]    number of bytes to transmit (instead of -t)\r\n");
    LOG_PRINTF("  -k, --blockcount #[KMG]   number of blocks (packets) to transmit (instead of -t or -n)\r\n");
    LOG_PRINTF("  -l, --len       #[KMG]    length of buffer to read or write\r\n");
    LOG_PRINTF("                            (default %d KB for TCP, %d Bytes for UDP)\r\n", DEFAULT_TCP_BLKSIZE/1024, DEFAULT_UDP_BLKSIZE);
    LOG_PRINTF("  -R, --reverse             run in reverse mode (server sends, client receives)\r\n");
    //LOG_PRINTF("  -w, --window    #[KMG]    set window size / socket buffer size\r\n");
    LOG_PRINTF("  -N, --no-delay            set TCP no delay, disabling Nagle's Algorithm\r\n");
    LOG_PRINTF("  -P, --parallel  #         number of parallel client streams to run\r\n");
    //LOG_PRINTF("  -O, --omit N              omit the first n seconds [0,%d]\r\n", MAX_OMIT);
    LOG_PRINTF("  -T, --title str           prefix every output line with this string\r\n");
}
...
/*
    ...
    */
    ...
    for(i=1; i<argc; i++) {
        if (ssv6xxx_strcmp(argv[i], "-p") == 0 || ssv6xxx_strcmp(argv[i], "--port") == 0 )
        {
            if (++i >= argc) {
                return -1;
            }
            test->server_port = ssv6xxx_atoi(argv[i]);
            continue;
        }
        else if (ssv6xxx_strcmp(argv[i], "-i") == 0 || ssv6xxx_strcmp(argv[i], "--interval") == 0 ){
            if (++i >= argc) {
                return -1;
            }
            test->stats_interval = test->reporter_interval = ssv6xxx_atoi(argv[i]);
        ...
        else if (ssv6xxx_strcmp(argv[i], "-c") == 0 || ssv6xxx_strcmp(argv[i], "--client") == 0 )
        {
            if (++i >= argc) {
                return -1;
            }
        ...
        else if (ssv6xxx_strcmp(argv[i], "-t") == 0 || ssv6xxx_strcmp(argv[i], "--time") == 0 )
        {
            if (++i >= argc) {
                return -1;
            }
            test->duration = ssv6xxx_atoi(argv[i]);
        ...
        else if (ssv6xxx_strcmp(argv[i], "-P") == 0 || ssv6xxx_strcmp(argv[i], "--parallel") == 0 )
        {
            if (++i >= argc) {
                return -1;
            }
            test->num_streams = ssv6xxx_atoi(argv[i]);
        ...
        else if (ssv6xxx_strcmp(argv[i], "-b") == 0 || ssv6xxx_strcmp(argv[i], "--bandwidth") == 0 )
        {
            if (++i >= argc) {
                return -1;
            }
            slash = (char * )ssv6xxx_strchr((const char *)argv[i], (char)'/');
            if (slash) {
                *slash = '\0';
                ++slash;
                test->settings->burst = ssv6xxx_atoi(slash);
        ...
        else if (ssv6xxx_strcmp(argv[i], "-n") == 0 || ssv6xxx_strcmp(argv[i], "--bytes") == 0 ){
            if (++i >= argc) {
                return -1;
            }
        ...
        else if (ssv6xxx_strcmp(argv[i], "-k") == 0 || ssv6xxx_strcmp(argv[i], "--blockcount") == 0 ){
            if (++i >= argc) {
                return -1;
            }
        ...
        /*
        else if (ssv6xxx_strcmp(argv[i], "-O") == 0 || ssv6xxx_strcmp(argv[i], "--omit") == 0 ){
            if (++i >= argc) {
                return -1;
            }
            test->omit = ssv6xxx_atoi(argv[i]);
        ...
        else if (ssv6xxx_strcmp(argv[i], "-T") == 0 || ssv6xxx_strcmp(argv[i], "--title") == 0 ){
            if (++i >= argc) {
                return -1;
            }
        ...
        /*
        else if (ssv6xxx_strcmp(argv[i], "-w") == 0 || ssv6xxx_strcmp(argv[i], "--window") == 0 ){
            if (++i >= argc) {
                return -1;
            }
        ...
        else if (ssv6xxx_strcmp(argv[i], "-l") == 0 || ssv6xxx_strcmp(argv[i], "--len") == 0 ){
            if (++i >= argc) {
                return -1;
            }
        ...
        else if (ssv6xxx_strcmp(argv[i], "-V") == 0 || ssv6xxx_strcmp(argv[i], "--verbose") == 0){
            test->verbose = 1;
            continue;
        }
        else if (ssv6xxx_strcmp(argv[i], "-v") == 0 || ssv6xxx_strcmp(argv[i], "--version") == 0 ){
            LOG_PRINTF("%s\r\n", iperf_version);
            return 100;
        }
        else if (ssv6xxx_strcmp(argv[i], "-s") == 0 || ssv6xxx_strcmp(argv[i], "--server") == 0 ){
            if (test->role == 'c') {
                test->i_errno = IESERVCLIENT;
                return -1;
            }
        ...
        else if (ssv6xxx_strcmp(argv[i], "-u") == 0 || ssv6xxx_strcmp(argv[i], "--udp") == 0 ){
            set_protocol(test, Pudp);
            client_flag = 1;
            continue;
        }
        else if (ssv6xxx_strcmp(argv[i], "-N") == 0 || ssv6xxx_strcmp(argv[i], "--no-delay") == 0 ){
            test->no_delay = 1;
            client_flag = 1;
            continue;
        }
        
        else if (ssv6xxx_strcmp(argv[i], "-R") == 0 || ssv6xxx_strcmp(argv[i], "--reverse") == 0 ){
            iperf_set_test_reverse(test, 1);
            client_flag = 1;
            continue;
        }
        else if (ssv6xxx_strcmp(argv[i], "-d") == 0 || ssv6xxx_strcmp(argv[i], "--debug") == 0 ){
            test->debug = 1;
            continue;
        }
        else
...
{
    if (test->debug)
        LOG_PRINTF("iperf state changed: %d --> %d\r\n", test->state, state);
...
iperf_check_throttle(struct iperf_stream *sp, struct timeval *nowP)
{
    ...
    if (msec == 0)
        return;
    bits_per_second = sp->result->bytes_sent_this_interval * 8 * 1000 / (ssv_type_u32)msec;    

    if (sp->test->debug){
        //LOG_PRINTF("check_id:%ld, msec:%ld, bits_per_sec:%u \r\n", check_id++, msec, bits_per_second);
    }
        ...
        SLIST_FOREACH(sp, &test->streams, streams) {
            if (sp->green_light &&
                ...
                if ((r = sp->snd(sp)) < 0) {
                    if (r == NET_SOFTERROR)
            			break;
            	    //LOG_PRINTF("iperf_send retval=%d \r\n", r);
...
get_parameters(struct iperf_test *test)
{
    ...
    if (j == NULL) {
        ...
        if ((j_p = cJSON_GetObjectItem(j, "sctp")) != NULL)
        {
            ...
            #else
            LOG_PRINTF("NOT support SCTP yet!\r\n");
            test->i_errno = IERECVPARAMS;
            cJSON_Delete(j);
            return -1;
            #endif
    	...
    	if ((j_p = cJSON_GetObjectItem(j, "omit")) != NULL){
    	    ...
    	    if (test->omit < 0 || test->omit > MAX_OMIT){
   	            LOG_PRINTF("omit time range: [0,%d] \r\n", MAX_OMIT);
                test->i_errno = IERECVPARAMS;
                cJSON_Delete(j);
                return -1;
    	    }
    	...
    	if ((j_p = cJSON_GetObjectItem(j, "MSS")) != NULL) 
    	{
            ...
            #else
            LOG_PRINTF("NOT support setting MSS option!\r\n");
            test->i_errno = IERECVPARAMS;
            cJSON_Delete(j);
            return -1;
            #endif    	    
    	...
    	if ((j_p = cJSON_GetObjectItem(j, "parallel")) != NULL)	{
    	    ...
    	    if (test->num_streams > MAX_STREAMS)
    	    {
   	            LOG_PRINTF("max streams in parallel allowed: %d \r\n", MAX_STREAMS);
                test->i_errno = IERECVPARAMS;
                cJSON_Delete(j);
                return -1;
    	    }
...
#if 0
    	    {
   	            LOG_PRINTF("NOT support reverse option in server mode \r\n");
                test->i_errno = IERECVPARAMS;
                cJSON_Delete(j);
                return -1;
    	    }
    	...
    	if ((j_p = cJSON_GetObjectItem(j, "flowlabel")) != NULL)  //for ipv6
    	    test->settings->flowlabel = j_p->valueint;
    	if ((j_p = cJSON_GetObjectItem(j, "title")) != NULL)
    	    test->title = iperf_strdup(j_p->valuestring);
    	if ((j_p = cJSON_GetObjectItem(j, "congestion")) != NULL)  //not used ?
...
send_results(struct iperf_test *test)
{
    ...
    if (j == NULL) {
    	...
    	if (test->role == 's' && test->get_server_output) {
    		...
    		OS_MemSET(output, 0, sizeof(buflen+1));
    		TAILQ_FOREACH(t, &(test->server_output_list), textlineentries) {
    		    ssv6xxx_strncat(output, t->line, buflen);
    		    buflen -= ssv6xxx_strlen(t->line);
    		}
...
JSON_write(int fd, cJSON *json)
{
    ...
    if (str == NULL)
    {
        LOG_PRINTF("cJSON_PrintUnformatted failed\r\n");
    	r = -1;
    }
    else {
    	hsize = ssv6xxx_strlen(str);
    	nsize = htonl(hsize);
...
add_to_interval_list(struct iperf_stream_result * rp, struct iperf_interval_results * new)
{
    ...
    if (irp != NULL){
        OS_MemCPY(irp, new, sizeof(struct iperf_interval_results));
...
iperf_new_test(void)
{
    ...
    OS_MemSET(test, 0, sizeof(struct iperf_test));
    ...
    OS_MemSET(test->settings, 0, sizeof(struct iperf_settings));
...
protocol_new(void)
{
    ...
    if(proto != NULL)
        OS_MemSET(proto, 0, sizeof(struct protocol));
...
iperf_defaults(struct iperf_test *testp)
{
    ...
    OS_MemSET(testp->cookie, 0, COOKIE_SIZE);
...
iperf_reset_test(struct iperf_test *test)
{
    ...
    test->settings->mss = 0;
    OS_MemSET(test->cookie, 0, COOKIE_SIZE);
...
iperf_stats_callback(struct iperf_test *test)
{
    ...
    SLIST_FOREACH(sp, &test->streams, streams) {
    	...
    	if (test->protocol->id == Ptcp) {
    	    ...
    	    if (test->debug){
        	    //LOG_PRINTF("sp->jit:%ld, tmp.packet count:%ld\r\n", sp->jitter, temp.interval_packet_count);
        	    //if (irp){
        	    //    LOG_PRINTF("irp->jit:%ld, irp->packet cout:%ld\r\n", irp->jitter, irp->interval_packet_count);
        	    //}
        	    //LOG_PRINTF("jitter: %ld\r\n", temp.jitter);
        	}
...
iperf_print_intermediate(struct iperf_test *test)
{
    ...
    ssv_type_u32 bandwidth;
    int retransmits = 0;
    ssv_type_u32 start_time, end_time;
    int total_packets = 0, lost_packets = 0;
    ssv_type_s32 lost_percent;
    ...
    SLIST_FOREACH(sp, &test->streams, streams) {
	    ...
	    if (irp == NULL) {
	        LOG_PRINTF("iperf_print_intermediate error: interval_results is NULL \r\n");
	        return;
	    }
...
iperf_print_results(struct iperf_test *test)
{
    ...
    ssv_type_u32   start_time, end_time, real_end_time, lost_percent;
    iperf_jit_t avg_jitter = 0;
    ssv_type_u32   bandwidth;
...
print_interval_results(struct iperf_test *test, struct iperf_stream *sp, cJSON *json_interval_streams)
{
    ...
    char nbuf[UNIT_LEN];
    char cbuf[UNIT_LEN];
    ssv_type_u32  st = 0, et = 0.;
    struct iperf_interval_results *irp = NULL;
    ssv_type_u32  bandwidth, lost_percent;

    irp = TAILQ_LAST(&sp->result->interval_results, irlisthead); /* get last entry in linked list */
    if (irp == NULL) {
    	LOG_PRINTF("print_interval_results error: interval_results is NULL \r\n");
        return;
    }
	/* First stream? */
...
iperf_new_stream(struct iperf_test *test, int s)
{
    ...
    OS_MemSET(sp, 0, sizeof(struct iperf_stream));
    ...
    sp->jitter = 0;
    OS_MemSET(sp->result, 0, sizeof(struct iperf_stream_result));
    ...
    OS_MemSET(sp->buffer, 'A', test->settings->blksize);
...
iperf_printf(struct iperf_test *test, const char* format, ...)
{
    ...
    if (test->role == 'c') {
    	if (test->title)
    	    LOG_PRINTF("%s:  ", test->title);
    	va_start(argp, format);
    	r = ssv6xxx_vsnprintf(linebuffer, sizeof(linebuffer), format, argp);
    	va_end(argp);
    	LOG_PRINTF("%s", linebuffer);
    }
    else if (test->role == 's') {
    	va_start(argp, format);
    	r = ssv6xxx_vsnprintf(linebuffer, sizeof(linebuffer), format, argp);
