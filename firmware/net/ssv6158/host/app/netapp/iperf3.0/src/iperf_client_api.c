...
iperf_handle_message_client(struct iperf_test *test)
{
    ...
    if (test->debug)
        LOG_PRINTF("iperf_handle_message_client read state: %d\r\n", test->state);
    switch (test->state) {
        case PARAM_EXCHANGE:
    ...
    if (test->debug)
        LOG_PRINTF("iperf_handle_message_client done \r\n");
...
iperf_connect(struct iperf_test *test)
{
    ...
    if (test->debug)
        LOG_PRINTF("iperf connect to server:%s, port:%u success \r\n", test->server_hostname, test->server_port);
...
iperf_run_client(struct iperf_test * test)
{
    ...
    startup = 1;
    OS_MemSET(&read_set, 0, sizeof(read_set));
    OS_MemSET(&write_set, 0, sizeof(write_set));
