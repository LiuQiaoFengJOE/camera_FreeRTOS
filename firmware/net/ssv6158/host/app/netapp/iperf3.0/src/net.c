...
netdial(int domain, int proto, char *local, int local_port, char *server, int port)
{
    ...
    if (local) {
        OS_MemSET(&l_sockaddr, 0, sizeof(l_sockaddr));
    ...
    OS_MemSET(&s_sockaddr, 0, sizeof(s_sockaddr));
...
netannounce(int domain, int proto, char *local, int port)
{
    ...
    OS_MemSET(&sock_addr, 0, sizeof(sock_addr));
...
/********************************************************************/
int
Nread(int fd, char *buf, ssv_type_size_t count, int prot)
{
    register int r;
    register ssv_type_size_t nleft = count;
    int sock_err;
...
int
Nread_partly(int fd, char *buf, ssv_type_size_t count, ssv_type_size_t *read_len, int prot)
{
    register int r;
    register ssv_type_size_t nleft = count;
    int sock_err;
    ...
    *read_len = 0;
    while (nleft > 0) {
        ...
        if (r < 0) {
            ...
            {
                LOG_PRINTF("Nread_partly err = %d r = %d\r\n", sock_err, r);
                return NET_HARDERROR;
            }
        } else if (r == 0)
            break;
...
int
Nwrite(int fd, const char *buf, ssv_type_size_t count, int prot)
{
    register int r;
    register ssv_type_size_t nleft = count;
    int sock_err;
...
int
Nwrite_partly(int fd, const char *buf, ssv_type_size_t count, ssv_type_size_t *written, int prot)
{
    register int r;
    register ssv_type_size_t nleft = count;
    int sock_err;
    ...
    *written = 0;
    while (nleft > 0) {
    	...
    	if (r < 0) {
    		...
    		default:
                {
                    LOG_PRINTF("Nwrite_partly err = %d r = %d\r\n", sock_err, r);
                    return NET_HARDERROR;
                }
    	    }
    	} else if (r == 0)
