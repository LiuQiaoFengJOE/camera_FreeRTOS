...
iperf_udp_recv(struct iperf_stream *sp)
{
    ...
    {
    	long pc;
    	OS_MemCPY(&sec, sp->buffer, sizeof(sec));
    	OS_MemCPY(&usec, sp->buffer+4, sizeof(usec));
    	OS_MemCPY(&pc, sp->buffer+8, sizeof(pc));
    	sec = ntohl(sec);
    	usec = ntohl(usec);
    	pcount = ntohl(pc);
    ...
    if (pcount >= sp->packet_count + 1) {
        ...
        if (sp->test->debug)
        {
            if (sp->packet_count == pcount)
                LOG_PRINTF("DUPLICATES - received packet = %d \r\n", sp->packet_count);
            else
        	    LOG_PRINTF("OUT OF ORDER - received packet = %d AND SP = %d, incoming packet = %u \r\n", sp->packet_count, sp->socket, pcount);
    	}
    }
    ...
    if (sp->prev_transit.tv_sec != 0 || sp->prev_transit.tv_usec != 0)
    {
        ...
        J = msecond_delta(&arrival_time, &sp->prev_receive);
        J += msecond_delta(&sp->prev_transit, &sent_time);
        if (J < 0)
            J = -J;
        sp->jitter += J;
        if (sp->test->debug)
        {
            //LOG_PRINTF("arr_sec:%ld, arr_usec:%ld, pre_sec:%ld, pre_usec:%ld \r\n", arrival_time.tv_sec, arrival_time.tv_usec, sp->prev_receive.tv_sec, sp->prev_receive.tv_usec);
            //LOG_PRINTF("snt_sec:%ld, snt_usec:%ld, pre_sec:%ld, pre_usec:%ld \r\n", sent_time.tv_sec, sent_time.tv_usec, sp->prev_transit.tv_sec, sp->prev_transit.tv_usec);
            //LOG_PRINTF("J=%ld, jitter:%ld \r\n", J, sp->jitter);
        }
    }
...
iperf_udp_send(struct iperf_stream *sp)
{
    ...
    {
    	ssv_type_u32  sec, usec, pcount;

    	sec = htonl(before.tv_sec);
    	usec = htonl(before.tv_usec);
    	pcount = htonl(sp->packet_count);
    	
    	OS_MemCPY(sp->buffer, &sec, sizeof(sec));
    	OS_MemCPY(sp->buffer+4, &usec, sizeof(usec));
    	OS_MemCPY(sp->buffer+8, &pcount, sizeof(pcount));
...
iperf_udp_accept(struct iperf_test *test)
{
     ...
     */
    if ((opt = test->settings->socket_bufsize) != NULL) {
