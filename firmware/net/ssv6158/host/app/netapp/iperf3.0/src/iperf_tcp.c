...
iperf_tcp_accept(struct iperf_test * test)
{
    ...
    if (ssv6xxx_strcmp(test->cookie, cookie) != 0) {
    ...
    if (test->no_delay || test->settings->mss || test->settings->socket_bufsize) {
        ...
        if (test->no_delay) {
            ...
            if( getsockopt(s, IPPROTO_TCP, TCP_NODELAY, &opt, &optlen) >= 0)
            {
                if (test->debug){
                    if( opt )
                        LOG_PRINTF( " sock(%d) TCP_NODELAY : ENABLED \r\n", s);
                    else
                        LOG_PRINTF( " sock(%d) TCP_NODELAY : DISABLED \r\n", s);
                }
            }
            else if (test->debug)
                    LOG_PRINTF( " sock(%d) getsockopt failed \r\n", s);
        }
