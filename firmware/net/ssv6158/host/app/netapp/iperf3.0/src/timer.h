...
extern Timer* tmr_create(
    ...
    ssv_type_s32 msecs, int periodic, void *iperf_test   );
...
*/
extern struct timeval* tmr_timeout( struct timeval* nowP, void *iperf_test   ) /* __attribute__((hot)) */;

/* Run the list of timers. Your main program needs to call this every so often,
** or as indicated by tmr_timeout().
*/
extern void tmr_run( struct timeval* nowP, void *iperf_test   ) /* __attribute__((hot)) */;

/* Reset the clock on a timer, to current time plus the original timeout. */
extern void tmr_reset( struct timeval* nowP, Timer* timer, void *iperf_test   );
...
*/
extern void tmr_cancel( Timer* timer, void *iperf_test   );

/* Clean up the timers package, freeing any unused storage. */
extern void tmr_cleanup( void *iperf_test   );

/* Cancel all timers and free storage, usually in preparation for exiting. */
extern void tmr_destroy( void *iperf_test  );

extern int iperf_gettimeofday(struct timeval*tv, void *tz);

extern ssv_type_u32 timeval_diff(struct timeval * tv0, struct timeval * tv1);
