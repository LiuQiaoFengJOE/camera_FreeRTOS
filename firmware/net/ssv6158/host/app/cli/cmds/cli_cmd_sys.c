...
void log_usage(void)
{
	...
	LOG_PRINTF("      module name = [all|mem|socket|api|tcp|udp|ip|other|data|ap|sta|httpd|dhcpd|netmgr|cmdeng|txrx|sniffer]\r\n");
}
...
{
    if( (argc == 2) &&(ssv6xxx_strcmp(argv[1], "msg") == 0))
    {
        msg_evt_show();
    }
    else if ( (argc == 2) &&(ssv6xxx_strcmp(argv[1], "trx") == 0))
    {
        ...
        if ((argc == 3) && (ssv6xxx_strcmp(argv[2],"show") == 0 )){
            ...
            if ((ssv6xxx_strcmp(argv[4],"all") == 0 ))
                ...
                module_bit = LOG_TXRX;
            else if((ssv6xxx_strcmp(argv[4],"sniffer") == 0 ))
                module_bit = LOG_SNIFFER;
#endif            
            else
            {;}
