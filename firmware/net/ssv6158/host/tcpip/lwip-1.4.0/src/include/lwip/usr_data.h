...
struct http_state {
  ...
  u8_t retries;
#if (defined(LWIP_HTTPD_SSI)  && LWIP_HTTPD_SSI)
  const char *parsed;     /* Pointer to the first unparsed byte in buf. */
#if (!defined(LWIP_HTTPD_SSI_INCLUDE_TAG) || !LWIP_HTTPD_SSI_INCLUDE_TAG)
...
#if (defined(LWIP_HTTPD_SSI_MULTIPART)  && LWIP_HTTPD_SSI_MULTIPART)
...
#endif /* LWIP_HTTPD_SSI */
#if (defined(LWIP_HTTPD_CGI)  && LWIP_HTTPD_CGI)
  char *params[LWIP_HTTPD_MAX_CGI_PARAMETERS]; /* Params extracted from the request URI */
  char *param_vals[LWIP_HTTPD_MAX_CGI_PARAMETERS]; /* Values for each extracted param */
#endif /* LWIP_HTTPD_CGI */
#if (defined(LWIP_HTTPD_DYNAMIC_HEADERS)  && LWIP_HTTPD_DYNAMIC_HEADERS)
  const char *hdrs[NUM_FILE_HDR_STRINGS]; /* HTTP headers to be sent. */
  u16_t hdr_pos;     /* The position of the first unsent header byte in the
                        current string */
  u16_t hdr_index;   /* The index of the hdr string currently being sent. */
#endif /* LWIP_HTTPD_DYNAMIC_HEADERS */
#if (defined(LWIP_HTTPD_TIMING)  && LWIP_HTTPD_TIMING)
  u32_t time_started;
#endif /* LWIP_HTTPD_TIMING */
#if LWIP_HTTPD_SUPPORT_POST
  u32_t post_content_len_left;
#if (defined(LWIP_HTTPD_POST_MANUAL_WND)  && LWIP_HTTPD_POST_MANUAL_WND)
  u32_t unrecved_bytes;
  struct tcp_pcb *pcb;
  u8_t no_auto_wnd;
#endif /* LWIP_HTTPD_POST_MANUAL_WND */
  ssv_type_s32 cgi_handler_index; 
  char *response_file; 
