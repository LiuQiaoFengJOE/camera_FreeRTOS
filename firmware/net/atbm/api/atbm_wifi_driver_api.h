...
#define WSM_MAX_NUM_LINK_AP 4//Lmac support station number is 4;
typedef struct _atbm_wifi_ap_info_{
    int rssi;
    unsigned long rx_packets;
    unsigned long tx_packets;
    unsigned long tx_suc_packets;
	atbm_uint16 tx_main_rate;
	atbm_uint16 rx_avg_rate;
    unsigned char  sta_mac[ATBM_ETH_ALEN];
    unsigned char  sta_cnt;
}atbm_wifi_ap_info;


/*
unsigned long rx_packets;  		¨º?¡ã¨¹¡Á¨¹¨ºy
unsigned long tx_packets;  		¡¤¡é¡ã¨¹¡Á¨¹¨ºy
unsigned long tx_suc_packets;	¡¤¡é¡ã¨¹3¨¦1|¨ºy
atbm_uint16 tx_main_rate;		¦Ì¡À?¡ã?¡Â¨°a¡¤¡é¡ã¨¹?¨´?¨º
atbm_uint16 rx_avg_rate;		¦Ì¡À?¡ã???¨´¨º?¡ã¨¹?¨´?¨º
...
typedef struct{
	atbm_int8 tx_link_rate[8];
	atbm_int8 rx_link_rate[8];
	atbm_uint32 tx_throughput;//kbps
	atbm_uint32 rx_throughput;//kbps
}DVR_EZWIFI_WIFI_RATE_SAMPLE_T;
...
struct TX_RX_Statistics_S{
	atbm_uint16 rx_rate_avg;//rx average rate
	atbm_uint32 received_num; //receive total
	atbm_uint32 sum_success_cnt;//transmit success total
	atbm_uint32 transmit_total_cnt;//transmit total
};
...
atbm_int32 atbm_wifi_set_rts(atbm_uint8 if_id,unsigned int rts_val);
...
int atbm_change_mac(struct atbmwifi_common *hw_priv, unsigned char *mac);
int atbm_change_deltagain(struct atbmwifi_common *hw_priv,
								unsigned char g1,unsigned char g2,unsigned char g3,int write_efuse);
int atbm_change_dcxo(struct atbmwifi_common *hw_priv,int dcxo,int write_efuse);
//int atbm_get_efuse(struct atbmwifi_common *hw_priv, unsigned char *efuse);

atbm_int32 atbm_wifi_set_edca_params(struct atbmwifi_vif *priv);

atbm_int32 atbm_wifi_rts_threhold_set(atbm_int32 val);
atbm_int32 atbm_wifi_rts_threhold_get(void);
atbm_int32 atbm_wifi_set_block_ack_policy(atbm_int8 val);
...
/****************************************************************************
* Function:     atbm_sta_rssi_get
*
* Purpose:      This function Obtaining the signal strength can also be used 
*				to determine whether to connect to the ap.
*
* Parameters:   type int 
*
* Returns:      Returns <=0 if succeed, otherwise a negative error code.
...
atbm_int32 atbm_sta_rssi_get(atbm_int32 *rssi);
/****************************************************************************
* Function:     atbm_wifi_get_channel_idle
*
* Purpose:      Gets the level of interference in the current wireless 
*				environment.
*				
* Parameters:   if_id=0
*
* Returns:      Returns >= 0 if succeed, otherwise a negative error code.
				return range[0,1000]
				The value ranges from 0 to 1000. If the value is less than 200, 
				the channel is very crowded. If the value is closer to 1000, 
				the environment is good
******************************************************************************/

atbm_int32 atbm_wifi_get_channel_idle(atbm_int32 if_id);
/****************************************************************************
* Function:     atbm_wifi_get_tx_throughput
*
* Purpose:      To get the current ideal throughput, 
*				multiply it by 65% for actual use.
*
* Parameters:   None
*
* Returns:      Returns >= 0 if succeed, otherwise a negative error code.
...
atbm_int32 atbm_wifi_get_tx_throughput(atbm_uint8 *bssid,atbm_int32 if_id);
/****************************************************************************
* Function:     atbm_wifi_get_txpacket_rate
*
* Purpose:      Gets the current main packet sending rate.
*
* Parameters:   None
*
* Returns:      Returns > 0 if succeed, otherwise a negative error code.
...
atbm_int32 atbm_wifi_get_txpacket_rate(atbm_uint8 *bssid,atbm_int32 if_id);

/****************************************************************************
* Function:     atbm_wifi_get_chip_temperature
*
* Purpose:      Gets the current chip operating temperature.
*
* Parameters:   None
*
* Returns:      Returns > 0 if succeed, otherwise a negative error code.
...
int atbm_wifi_get_chip_temperature(atbm_void);
/****************************************************************************
* Function:     atbm_wifi_get_ap_info
*
* Purpose:      Obtain sta connection status information in AP mode.
*
* Parameters:   buff :
					atbm_wifi_ap_info p_info[WSM_MAX_NUM_LINK_AP]
				if_id :
...
atbm_int8 atbm_wifi_get_ap_info(atbm_uint8 *buff,atbm_int32 if_id);
/****************************************************************************
...
atbm_int8 atbm_wifi_get_sta_info(atbm_uint8 *buff,atbm_int32 if_id);

/****************************************************************************
* Function:     atbm_set_firmware_flag
*				atbm_get_firmware_flag
* Purpose:      This function is used to set/get firmware flag
*
* Parameters:   flag
					0:only wifi 
					1:wifi_ble_comb 
					2:cert auth firmware
*
* Returns:      Returns >=0 if succeed, otherwise a negative error code.
******************************************************************************/

int atbm_set_firmware_flag(int flag);
int atbm_get_firmware_flag(void);
