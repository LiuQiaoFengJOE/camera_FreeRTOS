...
#define ATBMWIFI__LINK_ID_DTIM		(14)
...
struct atbmwifi_common{	
...
#if (PLATFORM == FH_EVIZE_RTT)
		atbm_mutex tx_lock;
		atbm_mutex appdata_lock;
		atbm_mutex scan_lock;
		atbm_mutex conn_ap_lock;
		atbm_mutex rty_conn_ap_lock;
		atbm_mutex dis_ap_lock;
		atbm_mutex ap_create_lock;
		atbm_mutex swich_mode_lock;
		atbm_mutex timer_lock;
		//atbm_mutex tx_lock;
#endif//(PLATFORM == FH_EVIZE_RTT)
	
	atbm_uint8 adaptive_started; // adaptive mode stop scan and beacon lost
	unsigned long	adaptive_connented;
	...
	unsigned long rx_aggr_map[BITS_TO_LONGS(ATBM_IEEE80211_MAX_RX_AGGR)];
	unsigned long tx_aggr_map[BITS_TO_LONGS(ATBM_IEEE80211_MAX_TX_AGGR)];
	...
	priv->chip_flag :
		...
		7:6132NB
	*/
	atbm_uint32 chip_flag;
	atbm_uint8 loader_ble;
	char *chip_name;
#if (PLATFORM == FH_EVIZE_RTT)	
	pAtbm_thread_t rate_sample_thread;
	atbm_uint8 quit_rate_sample_thread_flag;
	atbm_os_wait_queue_head_t wifi_rate_state_wake;
	
	DVR_EZWIFI_WIFI_RATE_SAMPLE_T wifi_rate_state[20];
#endif	
	//////////////////////////////////////
	//ETF
	...
	atbm_uint8   country_code[3];
	atbm_uint8 	 country_channel;
	atbm_uint8 ampdu_flag;
    struct efuse_headr efuse;
	ATBM_BOOL driver_init_regs;
	ATBM_BOOL driver_setup_ba;
	ATBM_BOOL lmc_wakeup_host;
	atbm_uint32 last_n_xmits;
...
#endif
					ht_operation_mode:1,
					special_scan:2,//0 nornal scan connect / 1 special scan no connect / 2 scan and connect need data 
...
#endif
#endif
	struct special_filter_table filter_table;
};
#ifdef fallthrough
...
typedef void (*atbm_user_event_callback)(unsigned int,int, void*);
int atbm_set_user_event_callback(atbm_user_event_callback cb);
