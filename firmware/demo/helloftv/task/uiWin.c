	...
	switch(msg->id)
	{
		case MSG_DIALOG_INIT:
			...
			if(win)
			{
				...
				debug_wrn("window open error,can not find registered msg!!!\n\n");
			break;
		case MSG_WIN_CHILE_DESTROY:
			...
			if(win)
			{
				...
				debug_wrn("child window close error,can not find registered msg!!!\n\n");
			break;
		case MSG_DIALOG_CLOSE:
			...
			if(win)
			{
				...
				debug_wrn("window close error,can not find registered msg!!!\n\n");
			break;
...
#define MAX_ARG       5
	...
	{
		debug_wrn("create window failed !!!\n");
		return INVALID_HANDLE;
	}
