...
#define GAME_BLOCK_BREAKER_PASS_DELAY	(1 << 3)	//(1 << 5)
...
void move_icon_universal_draw(DISPLAY_ICONSHOW icon, u8 *frame_buff, u8 *src, BOX_BOUNDARY boundary)
{
	...
	if(icon.pos_y < 0)
		icon.pos_y = 0;
	frame_icon_draw(frame_buff, src, &icon, NULL);
}
...
{
	game_bk_draw((u8 *)get_game_base_buf(), index + RES_GAME_BLOCK_BK0, NULL);
}
...
void flash_reinit(){
	...
	if(!ball)
	{
		printf("malloc fail %s %d,PortGetFreeHeapSize:%d\r\n", __func__, __LINE__, xPortGetFreeHeapSize());
		return ;
	}
...
static void flash_draw(){	
	...
	frame_buff_display();
	//printf("jk %s, %d, 0x%x\r\n", __func__, __LINE__, game_block_breaker_page_encode);
}


void game_block_key_active()
{
	u8 get_key = 0;//game_key_judge();
	static bool val = 0;
	val =!val;
	if(val)
		get_key = GAME_KEY_R;
