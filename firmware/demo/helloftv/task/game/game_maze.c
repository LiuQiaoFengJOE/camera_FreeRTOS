...
#define RECT_W 14//20//14//(TFT_HEIGHT/(MAP_W+2))
#define RECT_H 14//14//(TFT_HEIGHT/(MAP_H+2))
...
{
	maze_leve = 0;
	game_bk_draw((u8 *)get_game_base_buf(), 0, 0x8080/*NULL*/);
	maze_map_init();
}
	...
	while(1){
		...
		switch (msg){
			...
			case KEY_EVENT_POWEROFF:
				...
				printf("jk %s, %d ,KEY_EVENT_POWEROFF\n", __func__, __LINE__);
