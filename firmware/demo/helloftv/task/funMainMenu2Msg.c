...
static u8 show_bk;
static u32 mainmenu_bmp_id[MAINMENU_TASK_NUM]={RES_MAINMENU2_LENS,RES_MAINMENU2_PHOTO,RES_MAINMENU2_RECORD};
...
static int mainMenuKeyMsgOk(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	if(keyState==KEY_PRESSED)
	{
		...
		{
			SysCtrl.photoMode=PHOTO_LIF;
			functionStart(FUNCTION_NETTYPE,0);
		}
...
static int mainMenuKeyMsgUp(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	{
#if 1
		functionStart(FUNCTION_MAINMENU,1);
#else		
...
static int mainMenuKeyMsgDown(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	{
#if 1
		functionStart(FUNCTION_MAINMENU3,2);
#else		
		if(mainmenu_taskidx>2)
		{
			mainmenu_taskidx-=3;
...
static int mainMenuOpenWin(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	{
		jpgdecById( RES_MAINMENU2_BK, mainmenu_bk_buf/*,modeFreeSpace()*/);
...
WINDOW(mainMenu2Window,mainMenu2MsgDeal,mainMenu2Win)
