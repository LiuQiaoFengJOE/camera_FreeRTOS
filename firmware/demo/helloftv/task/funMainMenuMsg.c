...
static u8 show_bk;
static u32 mainmenu_bmp_id[MAINMENU_TASK_NUM]={RES_MAINMENU_PICID2,RES_MAINMENU_PICID,RES_MAINMENU_PICID3};
...
static int mainMenuKeyMsgOk(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	if(keyState==KEY_PRESSED)
	{
		...
		{
			SysCtrl.photoMode=PHOTO_SMALL_MODE;
			functionStart(FUNCTION_TAKE_PHOTO,0);
		}
		else if(1==mainmenu_taskidx)
		{
			SysCtrl.photoMode=PHOTO_IDCONTENT;
			SysCtrl.ailangid = PICID_LANGID_SCH;
			functionStart(FUNCTION_ROLE,0);
		//	SysCtrl.photoMode=PHOTO_NORMAL;
		//	functionStart(FUNCTION_TAKE_PHOTO,0);
		}
		else if(2==mainmenu_taskidx)
		{
			SysCtrl.photoMode = PHOTO_AI_CHAT;
			//SysCtrl.photoMode=PHOTO_IDCONTENT;
            //SysCtrl.aiuserRole =PICID_ROLE_QA;
			SysCtrl.ailangid = PICID_LANGID_SCH;
			functionStart(FUNCTION_TAKE_PHOTO,0);
...
#if 1	
#if 1
		functionStart(FUNCTION_MAINMENU3,1);
#else		
		extern void bt_recTest(void);
...
#if 1
#if 1
		functionStart(FUNCTION_MAINMENU2,2);
#else		
		extern void bt_sendTest(void);
		bt_sendTest();
#endif
#else	
		if(mainmenu_taskidx>2)
...
static int mainMenuOpenWin(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	{
		jpgdecById( RES_MAINMENU_BK, mainmenu_bk_buf/*,modeFreeSpace()*/);
