...
typedef enum
{
    ...
    CONFIG_ID_WIFIPREVIEWQUALITY,
    CONFIG_ID_WIFIPREVIERES,
    CONFIG_ID_VIDEOQUALITY,
    CONFIG_ID_PHOTOBURST,
    CONFIG_ID_PHOTOTIMELAPSE,
    CONFIG_ID_PHOTOTIMESTAMP,
	CONFIG_ID_SHARPNESS,
	CONFIG_ID_ISO,
	CONFIG_ID_DEBOUNCE,
	CONFIG_ID_DATEFORMAT,
	CONFIG_ID_PHOTOQUALITY,
	CONFIG_ID_WDR,
	CONFIG_ID_PICIDENTIFY,
...
#define _DEBG_CUT_SDR_DATA_              _DEBG_CUT_CSI_RAW_//_DEBG_CUT_DATA_DIS_//_DEBG_CUT_POST_YUV_LCD_ //control to cut source data or dma data
void sdram_data_capture_task(u8 *fname);

#define WIFI_SSID_LEN_MAX 	32 
#define WIFI_PWD_LEN_MAX 	16 
#define WIFI_MAC_LEN_MAX 	16 

typedef struct
{
	u8 		mode;				//1:ap mode,0:sta mode
	u8 		dhcpc_enable;		//sta mode dhcpc or static IP address
	u16 	re_cnn_timeout;	    //sta mode disconnect reconnect timeout
	u32		ip_addr0;   		//static client IP && server IP
	u32		ip_addr1;			//static client mask && server dhcpd begin
	u32		ip_addr2;			//static client gatway && server dhcpd end
	char 	mac[WIFI_MAC_LEN_MAX];			//static mac address
	char 	cfg_ssid[WIFI_SSID_LEN_MAX];			//static client join ap ssid && server ap ssid
	char 	pwd[WIFI_PWD_LEN_MAX];			//static client join ap password && server ap password
}network_config;
