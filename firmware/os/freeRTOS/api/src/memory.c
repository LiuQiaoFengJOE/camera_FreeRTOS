...
#define LWIP_MEM_ALIGN(addr) ((void *)(((_mem_ptr_t)(addr) + MEM_ALIGNMENT - 1) & ~(_mem_ptr_t)(MEM_ALIGNMENT - 1)))
...
#define LWIP_MEM_FREE_PROTECT() mutex_lock_legacy(mem_mutex);
#define LWIP_MEM_FREE_UNPROTECT() mutex_unlock_legacy(mem_mutex);
#define LWIP_MEM_ALLOC_DECL_PROTECT()
#define LWIP_MEM_ALLOC_PROTECT() mutex_lock_legacy(mem_mutex);
#define LWIP_MEM_ALLOC_UNPROTECT() mutex_unlock_legacy(mem_mutex);
#else
...
#define LWIP_MEM_ALLOC_DECL_PROTECT() 1
#define LWIP_MEM_ALLOC_PROTECT() 2
#define LWIP_MEM_ALLOC_UNPROTECT() 3
...
#if USE_MUTEX_LOCK
	mem_mutex = mutex_init_legacy_atbm(__FUNCTION__);
#endif
}
...
#if USE_MEM_ASSERT
	LWIP_ASSERT("mem_free: sanity check alignment", (((_mem_ptr_t)rmem) & (MEM_ALIGNMENT - 1)) == 0);

	LWIP_ASSERT("mem_free: legal memory", (unsigned char *)rmem >= (unsigned char *)ram && (unsigned char *)rmem < (unsigned char *)ram_end);
#else
	if (!(((((_mem_ptr_t)rmem) & (MEM_ALIGNMENT - 1)) == 0) && ((unsigned char *)rmem >= (unsigned char *)ram) && ((unsigned char *)rmem < (unsigned char *)ram_end)))
	{
		mem_slide_check(FALSE);
		p_err_fun;
		while (1)
...
#if USE_MEM_ASSERT
				LWIP_ASSERT("mem_malloc: allocated memory not above ram_end.", (_mem_ptr_t)mem + SIZEOF_STRUCT_MEM + size <= (_mem_ptr_t)ram_end);
				LWIP_ASSERT("mem_malloc: allocated memory properly aligned.", ((_mem_ptr_t)mem + SIZEOF_STRUCT_MEM) % MEM_ALIGNMENT == 0);
				LWIP_ASSERT("mem_malloc: sanity check alignment", (((_mem_ptr_t)mem) & (MEM_ALIGNMENT - 1)) == 0);
#else
			if (!(((_mem_ptr_t)mem + SIZEOF_STRUCT_MEM + size <= (_mem_ptr_t)ram_end) && ((((_mem_ptr_t)mem) & (MEM_ALIGNMENT - 1)) == 0) && (((_mem_ptr_t)mem + SIZEOF_STRUCT_MEM) % MEM_ALIGNMENT == 0)))
			{
				mem_slide_check(FALSE);
				p_err_fun;
				while (1)
...
void *mem_calloc_ex(const char *name, mem_size_t count, mem_size_t size)
{
	...
	p = pvPortMalloc(count * size, __func__); //(void*)mem_malloc_ex(name, count *size);
	if (p)
	{
		/* zero the memory */
...
#define const_data_range_ok(X) (((uint32_t)(X) < ((uint32_t)&_bss_start)) && ((uint32_t)(X) >= 0x20000000))

// #define alloc_mem_range_ok(X)  ((uint32_t)(X) < (0x20000000 + 0x20000) && ((uint32_t)(X) >= (uint32_t) &_bss_end))
#define alloc_mem_range_ok(X) ((uint32_t)(X) < ((uint32_t)&_bss_end + 0x20000) && ((uint32_t)(X) >= (uint32_t)&_bss_end))

#define is_mem_head_ok(X) ((alloc_mem_range_ok(X)) && ((X->next == 0) || alloc_mem_range_ok(X->next)) && ((X->prev == 0) || alloc_mem_range_ok(X->prev)) && ((X->used == 0) || (X->used == 1)) && alloc_mem_range_ok(X->name))
