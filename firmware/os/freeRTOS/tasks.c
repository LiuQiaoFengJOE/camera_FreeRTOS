...
/*-----------------------------------------------------------*/
void print_current_tcb_name()
{
	if (pxCurrentTCB)
	{
		TCB_t *pxNewTCB = pxCurrentTCB;
		printf("name:%s\n", pxNewTCB->pcTaskName);
	}
}
char* getCurTaskName(void)
{
	return pxCurrentTCB->pcTaskName;
}

uint32_t getCurTaskId(void)
{
	uint32_t flags;
	uint32_t id;
	flags=arch_local_irq_save();
	id=(( TCB_t * ) pxCurrentTCB)->id;
	arch_local_irq_restore(flags);
	return id;
}

#if (configSUPPORT_DYNAMIC_ALLOCATION == 1)
BaseType_t xTaskCreateWithID( TaskFunction_t pxTaskCode,
								const char * const pcName,
								uint16_t usStackDepth,
								void * const pvParameters,
								UBaseType_t uxPriority,
								TaskHandle_t * const pxCreatedTask ,uint32_t id) /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
		{
		TCB_t *pxNewTCB;
		BaseType_t xReturn;
		//usStackDepth= 1024*6;
		...
		//deg_Printf("%s %d\n",__func__,__LINE__);
			...
			if( pxNewTCB != NULL )
			{
				...
				prvAddNewTaskToReadyList( pxNewTCB );
				pxNewTCB->stackSize=usStackDepth;
				addToTaskQueue(pxNewTCB);
				xReturn = pdPASS;
			}
			else
			{
		...
		if (uxArraySize >= uxCurrentNumberOfTasks)
		{
...
#if (configGENERATE_RUN_TIME_STATS == 1)
			{
				if (pulTotalRunTime != NULL)
				{
					...
					*pulTotalRunTime = *pulTotalRunTime>>10;
					portCONFIGURE_TIMER_FOR_RUN_TIME_STATS();
					ulTaskSwitchedInTime = ulTotalRunTime=0;
#endif
				}
			}
#else
...
#if (configGENERATE_RUN_TIME_STATS == 1)
	{
		pxTaskStatus->ulRunTimeCounter = pxTCB->ulRunTimeCounter>>10;
