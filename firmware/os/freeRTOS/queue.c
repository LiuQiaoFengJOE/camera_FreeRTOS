 ...
 */
BaseType_t prvIsQueueEmpty(const Queue_t *pxQueue) PRIVILEGED_FUNCTION;
...
BaseType_t xQueueGenericSend(QueueHandle_t xQueue, const void *const pvItemToQueue, TickType_t xTicksToWait, const BaseType_t xCopyPosition)
{
	...
	for (;;)
	{
		...
		taskENTER_CRITICAL();
		{
			...
			queue is full. */
			if(startDebug&&(pxQueue->uxQueueNumber==7||pxQueue->uxQueueNumber==6||pxQueue->uxQueueNumber==5))
			{
				printf("[%s]send[%d],queue[%d],wait:%u,len:%u\n",getCurTaskName(),((unsigned int*)pvItemToQueue)[0],pxQueue->uxQueueNumber,pxQueue->uxMessagesWaiting,pxQueue->uxLength);
			}
...
BaseType_t xQueueGenericReceive(QueueHandle_t xQueue, void *const pvBuffer, TickType_t xTicksToWait, const BaseType_t xJustPeeking)
{
	...
	for (;;)
	{
		...
		taskENTER_CRITICAL();
		{
			if(startDebug&&(pxQueue->uxQueueNumber==7||pxQueue->uxQueueNumber==6||pxQueue->uxQueueNumber==5))
			{
				printf("[%s]recv,[%d]wait:%u,len:%u\n",getCurTaskName(),pxQueue->ucQueueType,pxQueue->uxMessagesWaiting,pxQueue->uxLength);
			}
			const UBaseType_t uxMessagesWaiting = pxQueue->uxMessagesWaiting;
